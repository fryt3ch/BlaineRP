...
namespace BlaineRP.Server.Game.Management.DoorSystem
{
    public class DoorSystem
    {
        ...
        public class Door
        {
            ...
            public static bool GetLockState(uint id) => Service.GetSharedData<bool?>($"DOORS_{id}_L") == true;

            public static void SetLockState(uint id, bool state, bool updateDb)
            {
                if (state)
                {
                    Service.SetSharedData($"DOORS_{id}_L", true);
                }
                else
                {
                    Service.ResetSharedData($"DOORS_{id}_L");
                }
            }
        }
