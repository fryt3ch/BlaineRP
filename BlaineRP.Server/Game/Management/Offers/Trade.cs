...
namespace BlaineRP.Server.Game.Management.Offers
{
    public partial class Trade
    {
        ...
        public (Items.Inventory.ResultTypes Result, PlayerData PlayerError) Execute(PlayerData pData, PlayerData tData)
        {
            if (pData.Cash < SenderMoney)
                return (Items.Inventory.ResultTypes.NotEnoughMoney, pData);

            if (tData.Cash < ReceiverMoney)
                return (Items.Inventory.ResultTypes.NotEnoughMoney, tData);
            ...
            if (senderFreeSlots + senderRemoveSlots < receiverItems.Count)
                return (Items.Inventory.ResultTypes.NoSpace, pData);
            ...
            if (receiverCurrentWeight - receiverRemoveWeight + senderRemoveWeight > Properties.Settings.Static.MAX_INVENTORY_WEIGHT)
                return (Items.Inventory.ResultTypes.NoSpace, tData);

            foreach (VehicleInfo x in SenderVehicles)
            {
                if (x.OwnerID != pData.CID)
                    return (Items.Inventory.ResultTypes.Error, null);
            }
            ...
            if (ReceiverVehicles.Count > 0 && pFreeVehSlots + SenderVehicles.Count - ReceiverVehicles.Count < 0)
            {
                ...
                return (Items.Inventory.ResultTypes.NotEnoughVehicleSlots, pData);
            }
            ...
            if (SenderVehicles.Count > 0 && tFreeVehSlots + ReceiverVehicles.Count - SenderVehicles.Count < 0)
            {
                ...
                return (Items.Inventory.ResultTypes.NotEnoughVehicleSlots, tData);
            }
            ...
            if (ReceiverBusinesses.Count > 0)
            {
                if (!pData.HasLicense(LicenseType.Business, true))
                {
                    ...
                    return (Items.Inventory.ResultTypes.NoBusinessLicense, pData);
                }
                ...
                if (pData.FreeBusinessesSlots + SenderBusinesses.Count - ReceiverBusinesses.Count < 0)
                {
                    ...
                    return (Items.Inventory.ResultTypes.NotEnoughBusinessSlots, pData);
                }
            }
            ...
            if (SenderBusinesses.Count > 0)
            {
                if (!tData.HasLicense(LicenseType.Business, true))
                {
                    ...
                    return (Items.Inventory.ResultTypes.NoBusinessLicense, tData);
                }
                ...
                if (tData.FreeBusinessesSlots + ReceiverBusinesses.Count - SenderBusinesses.Count < 0)
                {
                    ...
                    return (Items.Inventory.ResultTypes.NotEnoughBusinessSlots, tData);
                }
            }
            ...
            if (ReceiverGarages.Count > 0 && pData.FreeGaragesSlots + SenderGarages.Count - ReceiverGarages.Count < 0)
            {
                ...
                return (Items.Inventory.ResultTypes.NotEnoughGarageSlots, pData);
            }
            ...
            if (SenderGarages.Count > 0 && tData.FreeGaragesSlots + ReceiverGarages.Count - SenderGarages.Count < 0)
            {
                ...
                return (Items.Inventory.ResultTypes.NotEnoughGarageSlots, tData);
            }
            ...
            if (rHCount > 0)
            {
                if (pData.SettledHouseBase?.Type == Estates.HouseBase.Types.House)
                {
                    ...
                    return (Items.Inventory.ResultTypes.SettledToHouse, pData);
                }
                ...
                if (pData.FreeHouseSlots + sHCount - rHCount < 0)
                {
                    ...
                    return (Items.Inventory.ResultTypes.NotEnoughHouseSlots, pData);
                }
            }
            ...
            if (sHCount > 0)
            {
                if (pData.SettledHouseBase?.Type == Estates.HouseBase.Types.House)
                {
                    ...
                    return (Items.Inventory.ResultTypes.SettledToHouse, tData);
                }
                ...
                if (tData.FreeHouseSlots + rHCount - sHCount < 0)
                {
                    ...
                    return (Items.Inventory.ResultTypes.NotEnoughHouseSlots, tData);
                }
            }
            ...
            if (rHCount > 0)
            {
                if (pData.SettledHouseBase?.Type == Estates.HouseBase.Types.Apartments)
                {
                    ...
                    return (Items.Inventory.ResultTypes.SettledToApartments, pData);
                }
                ...
                if (pData.FreeApartmentsSlots + sHCount - rHCount < 0)
                {
                    ...
                    return (Items.Inventory.ResultTypes.NotEnoughApartmentsSlots, pData);
                }
            }
            ...
            if (sHCount > 0)
            {
                if (tData.SettledHouseBase?.Type == Estates.HouseBase.Types.Apartments)
                {
                    ...
                    return (Items.Inventory.ResultTypes.SettledToApartments, tData);
                }
                ...
                if (tData.FreeApartmentsSlots + rHCount - sHCount < 0)
                {
                    ...
                    return (Items.Inventory.ResultTypes.NotEnoughApartmentsSlots, tData);
                }
            }
                ...
                if (senderItems[i].ItemRoot is Items.IStackable senderItemS && senderItems[i].Amount < senderItemS.Amount)
                {
                    for (var j = 0; j < pData.Items.Length; j++)
                    {
                        if (pData.Items[j] == senderItems[i].ItemRoot)
                        {
                            ...
                            senderSlotsToUpdate.Add((j, Items.Item.ToClientJson(pData.Items[j], Items.Inventory.GroupTypes.Items)));
                ...
                if (receiverItems[i].ItemRoot is Items.IStackable receiverItemS && receiverItems[i].Amount < receiverItemS.Amount)
                {
                    for (var j = 0; j < tData.Items.Length; j++)
                    {
                        if (tData.Items[j] == receiverItems[i].ItemRoot)
                        {
                            ...
                            receiverSlotsToUpdate.Add((j, Items.Item.ToClientJson(tData.Items[j], Items.Inventory.GroupTypes.Items)));
            ...
            {
                pData.Player.InventoryUpdate(Items.Inventory.GroupTypes.Items, senderSlotsToUpdate[0].Item1, senderSlotsToUpdate[0].Item2);

                for (var i = 1; i < senderSlotsToUpdate.Count; i += 2)
                {
                    pData.Player.InventoryUpdate(Items.Inventory.GroupTypes.Items,
                        senderSlotsToUpdate[i].Item1,
                        senderSlotsToUpdate[i].Item2,
                        Items.Inventory.GroupTypes.Items,
            ...
            {
                tData.Player.InventoryUpdate(Items.Inventory.GroupTypes.Items, receiverSlotsToUpdate[0].Item1, receiverSlotsToUpdate[0].Item2);

                for (var i = 1; i < receiverSlotsToUpdate.Count; i += 2)
                {
                    tData.Player.InventoryUpdate(Items.Inventory.GroupTypes.Items,
            ...
            return (Items.Inventory.ResultTypes.Success, null);
        }
