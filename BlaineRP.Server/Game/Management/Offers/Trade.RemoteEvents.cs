    ...
    public partial class Trade
    {
        internal class RemoteEvents : Script
        {
            ...
            private static void Accept(Player player)
            {
                ...
                (Inventory.ResultTypes Result, PlayerData PlayerError) result = offer.TradeData.Execute(offer.Sender, offer.Receiver);

                if (result.Result == Items.Inventory.ResultTypes.Success)
                {
                ...
                {
                    if (result.Result == Items.Inventory.ResultTypes.Error)
                    {
                    ...
                    else if (result.Result == Items.Inventory.ResultTypes.NotEnoughMoney)
                    {
                    ...
                    else if (result.Result == Items.Inventory.ResultTypes.NoSpace)
                    {
                        if (pData == result.PlayerError)
                        {
            ...
            private static void UpdateItem(Player player, bool fromPockets, int slotTo, int slotFrom, int amount)
            {
                ...
                else
                {
                    ...
                    if (amount == iData.Amount)
                    {
                        ...
                        pData.Player.TriggerEvent("Inventory::Update", 10, -1, slotFrom, Items.Item.ToClientJson(null, Items.Inventory.GroupTypes.Items));
                        tData.Player.TriggerEvent("Inventory::Update", 12, slotFrom, Items.Item.ToClientJson(null, Items.Inventory.GroupTypes.Items));
                    }
                    else
                    {
                        iData.Amount -= amount;
