...
namespace BlaineRP.Server.Game.EntitiesData.Vehicles
{
    public partial class VehicleData
    {
        ...
        public RadioStationTypes Radio { get => (RadioStationTypes)Vehicle.GetSharedData<int>("Radio"); set { Vehicle.SetSharedData("Radio", (byte)value); } }
        ...
        public Vector4 FrozenPosition
        {
            get
            {
                if (Vehicle.GetSharedData<string>("IsFrozen") is string str)
                {
                    ...
                    return new Vector4(float.Parse(posData[0]), float.Parse(posData[1]), float.Parse(posData[2]), posData.Length > 3 ? float.Parse(posData[3]) : float.MinValue);
                }
