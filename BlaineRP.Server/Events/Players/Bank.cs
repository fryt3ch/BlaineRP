...
namespace BlaineRP.Server.Events.Players
{
    class Bank : Script
    {
        ...
        private static void BuyTariff(Player player, int bankId, int tariffNum)
        {
            ...
            if (!Enum.IsDefined(typeof(Game.Bank.Tariff.Types), tariffNum))
                return;

            var tariffType = (Game.Bank.Tariff.Types)tariffNum;

            var tariff = Game.Bank.Tariff.All[tariffType];
            ...
            if (pData.BankAccount == null)
            {
                ...
                pData.BankAccount = new Game.Bank.Account(pData.Info, tariffType);
