...
namespace BlaineRP.Server.Events.Players
{
    partial class Inventory : Script
    {
        ...
        private static void WorkbenchDrop(Player player, int groupNum, int slot, int amount)
        {
            ...
            if (group != Game.Items.Inventory.GroupTypes.CraftItems && group != Game.Items.Inventory.GroupTypes.CraftResult)
                return;
            ...
            var item = group == Game.Items.Inventory.GroupTypes.CraftItems ? wb.Items[slot] : wb.ResultItem;
            ...
            if (item is Game.Items.IStackable itemStackable)
            {
                ...
                if (curAmount > 0)
                {
                    ...
                    if (group == Game.Items.Inventory.GroupTypes.CraftItems)
                        wb.Items[slot] = item;
                    else
                        wb.ResultItem = item;
            ...
            var upd = Game.Items.Item.ToClientJson(group == Game.Items.Inventory.GroupTypes.CraftItems ? wb.Items[slot] : wb.ResultItem, group);
