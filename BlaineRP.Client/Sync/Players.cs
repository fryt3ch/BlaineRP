        ...
        public Players()
        {
            ...
            Events.Add("Players::CharacterPreload",
                async args =>
                {
                    ...
                    data.OwnedBusinesses = sData.TryGetValue("Businesses", out var value5)
                        ...
                        : new List<Locations.Business>();

                    data.OwnedHouses = sData.TryGetValue("Houses", out var value6)
                        ? Json.Deserialize<List<uint>>((string)value6).Select(x => Locations.House.All[x]).ToList()
                        : new List<Locations.House>();

                    data.OwnedApartments = sData.TryGetValue("Apartments", out var value7)
                        ? Json.Deserialize<List<uint>>((string)value7).Select(x => Locations.Apartments.All[x]).ToList()
                        : new List<Locations.Apartments>();

                    data.OwnedGarages = sData.TryGetValue("Garages", out var value8)
                        ? Json.Deserialize<List<uint>>((string)value8).Select(x => Locations.Garage.All[x]).ToList()
                        : new List<Locations.Garage>();
                    ...
                    if (sData.TryGetValue("SHB", out var value10))
                    {
                        ...
                        data.SettledHouseBase = (House.HouseTypes)int.Parse(shbData[0]) == House.HouseTypes.House
                            ? Locations.House.All[uint.Parse(shbData[1])]
                            : (Locations.HouseBase)Locations.Apartments.All[uint.Parse(shbData[1])];
                    }
                    ...
                    data.WeaponSkins = sData.TryGetValue("WSkins", out var value13)
                        ? Json.Deserialize<List<string>>((string)value13)
                              .ToDictionary(x =>
                                       ((Items.WeaponSkin.ItemData)Items.GetData(x, typeof(Items.WeaponSkin))).Type,
                                   x => x)
                        : new Dictionary<Items.WeaponSkin.ItemData.Types, string>();
            ...
            Events.Add("Player::WSkins::Update",
                args =>
                {
                    ...
                    var type = ((Items.WeaponSkin.ItemData)Items.GetData(id)).Type;
            ...
            Events.Add("Player::SettledHB",
                args =>
                {
                    var pType = (House.HouseTypes)(int)args[0];

                    var pId = (uint)(int)args[1];

                    var state = (bool)args[2];

                    var house = pType == House.HouseTypes.House
                        ? Locations.House.All[pId]
                        : (Locations.HouseBase)Locations.Apartments.All[pId];
            ...
            Events.Add("Player::Properties::Update",
                args =>
                {
                    ...
                    else if (pType == PropertyTypes.Apartments)
                    {
                        ...
                        var t = Locations.Apartments.All[hid];
                    ...
                    else if (pType == PropertyTypes.Garage)
                    {
                        ...
                        var t = Locations.Garage.All[gid];
                    ...
                    else if (pType == PropertyTypes.Business)
                    {
                        ...
                        var t = Locations.Business.All[bid];
        ...
        public static async void TryShowWeaponSkinsMenu()
        {
            ...
            await ActionBox.ShowSelect("WeaponSkinsMenuSelect",
                ...
                wSkins.Select(x => ((decimal)x.Key,
                           $"{Locale.Actions.WeaponSkinTypeNames.GetValueOrDefault(x.Key) ?? "null"} | {Items.GetName(x.Value).Split(' ')[0]}"))
                      .ToArray(),
