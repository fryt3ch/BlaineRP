...
namespace BlaineRP.Client.Sync.Quests.Types.Job
{
    ...
    internal class JTR1
    {
        public JTR1()
        {
            new Quest.QuestData(Enums.QuestTypes.JTR1, "Доставка груза", "Дальнобойщик", new Dictionary<byte, Quest.QuestData.StepData>()
            {
                {
                    0,
...

                {
                    ...
                    new Quest.QuestData.StepData("Заберите груз со склада", 1)
                    {
                        StartAction = (pData, quest) =>
                        {
                            ...
                            var currentOrder = new Data.Jobs.Trucker.OrderInfo() { Id = uint.Parse(qData[0]), MPIdx = int.Parse(qData[1]), TargetBusiness = Locations.Business.All[int.Parse(qData[2])], Reward = uint.Parse(qData[3]) };
