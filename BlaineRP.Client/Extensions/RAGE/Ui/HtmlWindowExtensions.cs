...
namespace BlaineRP.Client.Extensions.RAGE.Ui
{
    public static class HtmlWindowExtensions
    {
        ...
        public static void ExecuteJs(this HtmlWindow window, string function, params object[] args) => window?.ExecuteJs($"{function}({string.Join(", ", args.Select(x => global::RAGE.Util.Json.Serialize(x)))});");
        ...
        public static void ExecuteCachedJs(this HtmlWindow window, string function, params object[] args) => window?.ExecuteCachedJs($"{function}({string.Join(", ", args.Select(x => global::RAGE.Util.Json.Serialize(x)))});");
    }
}