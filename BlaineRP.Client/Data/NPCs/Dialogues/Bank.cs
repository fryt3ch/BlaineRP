        ...
        public Bank()
        {
            new Dialogue("bank_preprocess", null,
                ...
                async (args) =>
                {
                    ...
                    if (hasAccount)
                    {
                        btns.Add(new Dialogue.Button("[Перейти к управлению счетом]", () =>
                        {
                    ...
                    {
                        btns.Add(new Dialogue.Button("Да, хочу стать клиентом вашего банка", () => { NPC.CurrentNPC?.ShowDialogue("bank_no_account_1"); }));
                    }

                    if (pData.OwnedHouses.FirstOrDefault() is Data.Locations.House house)
                        btns.Add(new Dialogue.Button("[Счет дома]", () => { NPC.CurrentNPC?.ShowDialogue("bank_preprocess_house", false, new object[] { house }); }));

                    if (pData.OwnedApartments.FirstOrDefault() is Data.Locations.Apartments aps)
                        btns.Add(new Dialogue.Button("[Счет квартиры]", () => { NPC.CurrentNPC?.ShowDialogue("bank_preprocess_aps", false, new object[] { aps }); }));

                    if (pData.OwnedGarages.FirstOrDefault() is Data.Locations.Garage garage)
                        btns.Add(new Dialogue.Button("[Счет гаража]", () => { NPC.CurrentNPC?.ShowDialogue("bank_preprocess_garage", false, new object[] { garage }); }));

                    if (pData.OwnedBusinesses.FirstOrDefault() is Data.Locations.Business biz)
                        btns.Add(new Dialogue.Button("[Счет бизнеса]", () => { NPC.CurrentNPC?.ShowDialogue("bank_preprocess_business", false, new object[] { biz }); }));

                    if (pData.CurrentFraction is Data.Fractions.Fraction fraction)
                        btns.Add(new Dialogue.Button("[Счет фракции]", () => { NPC.CurrentNPC?.ShowDialogue("bank_preprocess_fraction", false, new object[] { fraction }); }));
                ...
                async (args) =>
            {
                ...
                btns.Add(new Dialogue.Button("[Пополнить счет]", async () =>
                {
                    if (NPC.CurrentNPC == null)
                        return;
                ...
                if (balance > 0)
                {
                    btns.Add(new Dialogue.Button("[Снять со счета]", async () =>
                    {
                        if (NPC.CurrentNPC == null)
                            return;
                ...
                btns.Add(Dialogue.Button.DefaultBackButton);
                btns.Add(Dialogue.Button.DefaultExitButton);
            ...
            new Dialogue("bank_no_account_1", "Отлично!\nУ нас есть несколько выгодных тарифов, ознакомьтесь с ними и выберите интересующий", null,
                ...
                new Dialogue.Button("[Перейти к тарифам]", () =>
                {
                    if (NPC.CurrentNPC?.Data is Data.Locations.Bank bankData)
                    {
                        if (NPC.LastSent.IsSpam(1000, false, false))
