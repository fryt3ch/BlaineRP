...
namespace BlaineRP.Client.Utils.Game
{
    internal static class Misc
    {
        ...
        {
            return RAGE.Game.Pad.IsControlJustPressed(0, 32) || RAGE.Game.Pad.IsControlJustPressed(0, 33) || RAGE.Game.Pad.IsControlJustPressed(0, 34) || RAGE.Game.Pad.IsControlJustPressed(0, 35) || RAGE.Game.Pad.IsControlJustPressed(0, 22) || RAGE.Game.Pad.IsControlJustPressed(0, 44) || RAGE.Game.Pad.IsControlJustPressed(0, 75);
        }
        ...
        public static List<Ped> GetPedsOnScreen(int maxCount = 5) => Entities.Peds.Streamed.Where(x => x.IsOnScreen()).OrderBy(x => Vector3.Distance(x.Position, Player.LocalPlayer.Position)).Take(maxCount).ToList();
        ...
        public static List<Vehicle> GetVehiclesOnScreen(int maxCount = 5) => Entities.Vehicles.Streamed.Where(x => x.IsOnScreen()).OrderBy(x => Vector3.Distance(x.Position, Player.LocalPlayer.Position)).Take(maxCount).ToList();
        ...
        {
            if (!RAGE.Game.Ui.IsWaypointActive()) return 0;
            ...
            for (var i = firstBlip; RAGE.Game.Ui.DoesBlipExist(i); i = nextBlip)
            {
