...
namespace BlaineRP.Client.Utils
{
    internal static class Geometry
    {
        ...
        {
            return (float)(180f / System.Math.PI) * radians;
        }
        ...
        public static Vector3 RotationToDirection(Vector3 rotation)
        {
            ...
            double num = System.Math.Abs(System.Math.Cos(x));
        ...
        public static Vector3 GetRotationToFacePointTo(Vector3 position, Vector3 target)
        {
            ...
            float pitch = RadiansToDegrees((float)System.Math.Atan2(direction.Z, System.Math.Sqrt(direction.X * direction.X + direction.Y * direction.Y)));
            float roll = RadiansToDegrees((float)System.Math.Atan2(direction.Z, System.Math.Sqrt(direction.Y * direction.Y + direction.Z * direction.Z)));
            float yaw = RadiansToDegrees((float)System.Math.Atan2(direction.Y, direction.X)) - 90f; // subtract 90f to fit game coord system

            return new Vector3(pitch, roll, yaw);
        }

        public static float GetRotationZToFacePointTo(Vector3 position, Vector3 target)
        {
            return RadiansToDegrees((float)System.Math.Atan2(target.Y - position.Y, target.X - position.X)) - 90f;
        }
