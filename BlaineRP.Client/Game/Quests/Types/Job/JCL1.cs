...
namespace BlaineRP.Client.Game.Quests
{
    ...
    internal class JCL1
    {
        public JCL1()
        {
            new Quest.QuestData(QuestTypes.JCL1,
                "Доставка денег",
                "Инкассатор",
                new Dictionary<byte, Quest.QuestData.StepData>()
                {
                    ...
                    {
                        1, new Quest.QuestData.StepData("Заберите деньги", 1)
                        {
                            StartAction = (pData, quest) =>
                            {
                                ...
                                var currentOrder = new Collector.OrderInfo()
                                {
                                    Id = uint.Parse(qData[0]),
                                    TargetBusiness = Business.All[int.Parse(qData[1])],
                                    Reward = uint.Parse(qData[2]),
                                };
                                ...
                                var destPos = new Vector3(currentOrder.TargetBusiness.InfoColshape.Position.X,
                                    ...
                                    currentOrder.TargetBusiness.InfoColshape.Position.Z - 1f
                                );
                                ...
                                var colshape = new Cylinder(destPos, 5f, 10f, true, new Colour(255, 0, 0, 125), Settings.App.Static.MainDimension, null)
                                {
                                    ...
                                    OnEnter = (cancel) =>
                                    {
                                        ...
                                        var scaleform = Scaleform.CreateCounter("job_collector_0",
                                            Locale.Get("SCALEFORM_JOB_COLLECTOR_WAIT_0_HEADER"),
                    ...
                    {
                        2, new Quest.QuestData.StepData("Доставьте деньги в банк", 1)
                        {
                            StartAction = (pData, quest) =>
                            {
                                string[] qData = quest.CurrentData?.Split('&');
                                ...
                                var colshape = new Cylinder(destPos, 5f, 10f, true, new Colour(255, 0, 0, 125), Settings.App.Static.MainDimension, null)
                                {
                                    ...
                                    OnEnter = (cancel) =>
                                    {
                                        ...
                                        var scaleform = Scaleform.CreateCounter("job_collector_0",
                                            Locale.Get("SCALEFORM_JOB_COLLECTOR_WAIT_1_HEADER"),
                                            Locale.Get("SCALEFORM_JOB_TRUCKER_WAIT_CONTENT"),
                                            waitTime / 1000,
                                            Scaleform.CounterSoundTypes.None
                                        );
