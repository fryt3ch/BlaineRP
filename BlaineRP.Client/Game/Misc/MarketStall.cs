    ...
    public class MarketStall
        {
                            ...
                            async (ActionBox.ReplyTypes rType, decimal opt) =>
                            {
                                ...
                                else if (opt == 3)
                                {
                                    ...
                                    for (int i = 0; i < sellHist.Count; i++)
                                    {
                                        ...
                                        strings.Add(Locale.Get("MARKETSTALL_SH_0", i + 1, Game.Items.Core.GetName(x.Item2), x.Item3, Locale.Get("GEN_MONEY_0", x.Item4)));
                                    }
            ...
            private static async void ShowGoods(MarketStall marketStall)
            {
                ...
                foreach (var x in resList)
                {
                    ...
                    var iType = Game.Items.Core.GetType(id, true);

                    var iName = Game.Items.Core.GetNameWithTag(id, iType, tag, out _);

                    items.Add(new object[] { new object[] { uint.Parse(d[0]), Game.Items.Core.GetImageId(id, iType) }, iName, price, amount, weight, typeof(Clothes).IsAssignableFrom(iType) ? Locale.Get("SHOP_RET_DRESS_L") : null });
                }
