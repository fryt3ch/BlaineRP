...
namespace BlaineRP.Client.Game.UI.CEF
{
    ...
    public class Notification
    {
        ...
        private static Dictionary<string, Instance> Prepared = new Dictionary<string, Instance>()
        {
            ...
            {
                "ACMD::NA",
                new Instance(Types.Error, "Для использования данной команды необходим более высокий уровень администрирования!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            {
                "Str::NM",
                new Instance(Types.Error, "Введенная строка содержит недопустимые символы либо длиннее/короче, чем должна быть!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            ...
            {
                "ASTUFF::PAHMW",
                new Instance(Types.Error, "У данного игрока уже есть максимальное кол-во предупреждений ({0})!\nЕго следует заблокировать", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            ...
            {
                "KickB",
                new Instance(Types.Ban,
                    "Вы были заблокированы на сервере!\nАдминистратор: {0}\nПричина: {1}\nДата окончания блокировки: {2}\n\nВы можете обжаловать решение Администрации сервера на нашем форуме!",
            ...
            {
                "Business::MMB",
                new Instance(Types.Error,
                    "На складе Вашего бизнеса не может находиться больше, чем {0} ед. материалов!\nСейчас там находится {1} ед. материалов",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            {
                "Business::COIT",
                new Instance(Types.Error, "Вы не можете отменить этот заказ, так как он уже находится в процессе доставки!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
...
/*            { "Mute", new Instance(Types.Mute, Locale.Notifications.Punishments.Mute.Header, Locale.Notifications.Punishments.GotTimed) },
            ...
            {
                "Trade::EOP",
                new Instance(Types.Error, "Другой игрок не может что-то передать Вам сейчас или получить от Вас, спросите его об этом", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            ...
            { "Trade::MBIT", new Instance(Types.Error, "Вы не можете выбрать больше, чем {0} бизнеса для обмена", Locale.Get("NOTIFICATION_HEADER_ERROR")) },
            {
                "Trade::MGPH",
                new Instance(Types.Error,
                    "Чтобы продать/обменять гараж #{0}, на его счете должно быть как минимум столько средств, чтобы было достаточно для оплаты {1} часов налога",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            {
                "Trade::MBPH",
                new Instance(Types.Error,
                    "Чтобы продать/обменять бизнес #{0}, на его счете должно быть как минимум столько средств, чтобы было достаточно для оплаты {1} часов аренды",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            {
                "Trade::MHPH",
                new Instance(Types.Error,
                    "Чтобы продать/обменять дом #{0}, на его счете должно быть как минимум столько средств, чтобы было достаточно для оплаты {1} часов налога",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            {
                "Trade::MAPH",
                new Instance(Types.Error,
                    "Чтобы продать/обменять квартиру #{0}, на ее счете должно быть как минимум столько средств, чтобы было достаточно для оплаты {1} часов налога",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            {
                "Trade::MVOW",
                new Instance(Types.Error,
                    "Вы уже владеете максимальным кол-вом транспорта - {0}\nЧтобы увеличить это кол-во, купите дом или гараж с нужным кол-вом гаражных мест",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            ...
            { "Trade::MBOW", new Instance(Types.Error, "Вы уже владеете максимальным кол-вом бизнесов - {0}", Locale.Get("NOTIFICATION_HEADER_ERROR")) },
            {
                "Trade::ASH",
                new Instance(Types.Error,
                    "Вы уже прописаны в каком-то доме, чтобы владеть своим домом, вы не должны быть прописаны в другом!",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            {
                "Trade::ASA",
                new Instance(Types.Error,
                    "Вы уже прописаны в какой-то квартире, чтобы владеть своей квартирой, вы не должны быть прописаны в другой!",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            ...
            {
                $"Lic::N_{(int)LicenseTypes.A}",
                new Instance(Types.Error, "У Вас отсутствует водительская лицензия категории A (мотоциклы)!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            {
                $"Lic::N_{(int)LicenseTypes.B}",
                new Instance(Types.Error, "У Вас отсутствует водительская лицензия категории B (легковой транспорт)!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            {
                $"Lic::N_{(int)LicenseTypes.C}",
                new Instance(Types.Error, "У Вас отсутствует водительская лицензия категории C (грузовой транспорт)!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            {
                $"Lic::N_{(int)LicenseTypes.D}",
                new Instance(Types.Error, "У Вас отсутствует водительская лицензия категории D (маршрутный транспорт)!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            {
                $"Lic::N_{(int)LicenseTypes.Sea}",
                new Instance(Types.Error, "У Вас отсутствует водительская лицензия категории Sea (водный транспорт)!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            {
                $"Lic::N_{(int)LicenseTypes.Fly}",
                new Instance(Types.Error, "У Вас отсутствует водительская лицензия категории Air (воздушный транспорт)!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            ...
            {
                "Report::N",
                new Instance(Types.Success,
                    "Ваше сообщение принято, ответ будет дан администрацией в ближайшее время!\n\nС каждым новым отправленным сообщением Ваш запрос будет перемещен в конец очереди.\n\nНомер Вашего запроса в очереди: {0}",
                    Locale.Get("NOTIFICATION_HEADER_DEF")
                )
            },
            ...
            { "Fraction::RE0", new Instance(Types.Error, "Вы не можете редактировать эту должность!", Locale.Get("NOTIFICATION_HEADER_ERROR")) },
            {
                "Fraction::RE1",
                new Instance(Types.Error,
                    "Вы не можете самостоятельно изменить название должности!\nОставьте заявку на форуме с желаемыми названиями должностей и ожидайте ответа",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            {
                "Fraction::HRIBTY",
                new Instance(Types.Error, "Должность этого сотрудника равна либо выше Вашей, вы не можете сделать это!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            ...
            { "Fraction::FN", new Instance(Types.Information, "Вы были уволены из фракции!", Locale.Get("NOTIFICATION_HEADER_DEF")) },
            {
                "Fraction::CWL",
                new Instance(Types.Error,
                    "Создание предметов из материалов на данный момент недоступно!\nОбратитесь к руководству фракции",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            {
                "Fraction::NEWSMC",
                new Instance(Types.Error, "Достигнуто максимальное кол-во активных новостей, удалите любую старую новость!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            ...
            { "Police::DBS::PAF", new Instance(Types.Error, "Вы сейчас и так просматриваете сведения об этом человеке!", Locale.Get("NOTIFICATION_HEADER_ERROR")) },
            {
                "Police::APB::NYD",
                new Instance(Types.Error,
                    "Вы не можете исполнить (удалить) эту ориентировку, т.к. она была создана сотрудником другого департамента!",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            {
                "Police::GPSTR::NYD",
                new Instance(Types.Error,
                    "Вы не можете отключить этот GPS-трекер, т.к. он был активирован сотрудником другого департамента!",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            ...
            {
                "DriveS::NPTT",
                new Instance(Types.Error, "Вы не проходили теоритический тест для этого типа транспорта, чтобы сдавать практический!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
            {
                "DriveS::AHPT",
                new Instance(Types.Error,
                    "Вы уже сдали теоретическую часть одного теста, завершите его практическую часть, чтобы начать сдавать другой!",
                    Locale.Get("NOTIFICATION_HEADER_ERROR")
                )
            },
            ...
            { "DriveS::TTS", new Instance(Types.Success, "Вы сдали теоретическую часть теста!\nНабрано баллов: {0} из {1} минимальных", Locale.Get("NOTIFICATION_HEADER_DEF")) },
            {
                "DriveS::PEF0",
                new Instance(Types.Error,
                    "Вы провалили практическую часть экзамена!\nЕсли хотите повторить попытку, придется заново оплатить и сдать теорию!",
                    Locale.Get("NOTIFICATION_HEADER_DEF")
                )
            },
            {
            ...
            },
            {
                "DriveS::PES",
                new Instance(Types.Success,
                    "Вы успешно сдали экзамен и получаете права категории {0}!\nНе нарушайте ПДД, за серьезные нарушения Вы можете лишиться прав!",
                    Locale.Get("NOTIFICATION_HEADER_DEF")
                )
            },
            ...
            {
                "Casino::BLJAP",
                new Instance(Types.Error, "Вы не можете сесть, т.к. за этим столом уже идёт игра, в которой Вы не участвуете!", Locale.Get("NOTIFICATION_HEADER_ERROR"))
            },
        ...
        public Notification()
        {
            LastAntiSpamShowed = World.Core.ServerTime;

            Events.Add("Notify::Custom",
                (args) =>
                {
            ...
            Events.Add("Item::Added",
                (object[] args) =>
                {
                    Show(Types.Item,
                        Locale.Notifications.Inventory.Header,
                        (int)args[1] > 1
                            ? string.Format(Locale.Notifications.Inventory.Added, Items.Core.GetName((string)args[0]), (int)args[1])
                            : string.Format(Locale.Notifications.Inventory.AddedOne, Items.Core.GetName((string)args[0]))
                    );
                }
            );
            ...
            Events.Add("Item::FCN",
                (object[] args) =>
                {
                    Show(Types.Information,
                        Locale.Notifications.Inventory.FishingHeader,
                        (int)args[1] > 1
                            ? string.Format(Locale.Notifications.Inventory.FishCatched, Items.Core.GetName((string)args[0]), (int)args[1])
                            : string.Format(Locale.Notifications.Inventory.FishCatchedOne, Items.Core.GetName((string)args[0]))
                    );
                }
            );
            ...
            Events.Add("Notify::P",
                (object[] args) =>
                {
                    List<object> args1 = ((Newtonsoft.Json.Linq.JArray)args[2]).ToObject<List<object>>();

                    Player player = Entities.Players.GetAtRemote(Utils.Convert.ToUInt16(args[1]));
        ...
        public static bool IsActive => Browser.IsActive(Browser.IntTypes.Notifications);
        ...
        {
            Instance inst = Prepared.GetValueOrDefault(type);
        ...
        public static void Show(Types type, string title, string content, int timeout = -1)
        {
            ...
            Browser.Window.ExecuteJs("Notific.draw",
                timeout <= 0 ? GetTextReadingTime(title + content) : timeout,
                type.ToString(),
                Utils.Misc.ReplaceNewLineHtml(title),
                Utils.Misc.ReplaceNewLineHtml(content),
                MaxNotifications,
                false
            );
        }
        ...
        public static void ShowHint(string content, bool showAnyway = false, int timeout = -1)
        {
            ...
            Browser.Window.ExecuteJs("Notific.draw",
                timeout <= 0 ? GetTextReadingTime(content) : timeout,
                Types.Information.ToString(),
                Locale.Notifications.Hints.Header,
                Utils.Misc.ReplaceNewLineHtml(content),
                MaxNotifications,
                false
            );
        ...
        public static void ShowOffer(string content, int timeout = 9999999)
        {
            ...
            Browser.Window.ExecuteJs("Notific.draw",
                timeout,
                Types.Offer.ToString(),
                Locale.Notifications.Offers.Header,
                Utils.Misc.ReplaceNewLineHtml(content),
                MaxNotifications,
                true
            );
        ...
        {
            bool spam = World.Core.ServerTime.Subtract(dateTime).TotalMilliseconds < timeout;

            if (spam && notify && World.Core.ServerTime.Subtract(LastAntiSpamShowed).TotalMilliseconds > 500)
            {
                Show(Types.Error, Locale.Get("NOTIFICATION_HEADER_ASPAM"), Locale.Get("ANTISPAM_TEXT_0"), 2000);

                LastAntiSpamShowed = World.Core.ServerTime;
            }

            if (updateTime)
                dateTime = World.Core.ServerTime;
        ...
        {
            int optimalTime = text.Where(x => char.IsLetterOrDigit(x)).Count() * 100;
        ...
        {
            string tName = FiveNotificImgNames.GetValueOrDefault(smsType);
        ...
        {
            string tName = FiveNotificImgNames.GetValueOrDefault(FiveNotificImgTypes.IncomingCall);
        ...
        public static void ShowFiveNotification(string imageDict,
                                                string imageName,
                                                int iconType,
                                                string label,
                                                string subject,
                                                ...
                                                float durationCoef)
        {
            int curNotific = RAGE.Game.Ui.GetCurrentNotification();
        ...
        {
            if (!Browser.IsRendered(Browser.IntTypes.Notifications))
                return;

            IsOnTop = state;

            Browser.Window.ExecuteJs("Notific.switchPos", state);
        }
        ...
        public static bool HasApproveTimedOut(string context, DateTime curTime, int timeoutMs)
        {
            return context != ApproveContext || curTime.Subtract(ApproveTime).TotalMilliseconds >= timeoutMs;
        }
