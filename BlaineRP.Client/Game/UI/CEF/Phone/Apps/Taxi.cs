...
namespace BlaineRP.Client.Game.UI.CEF.Phone.Apps
{
    ...
    public class Taxi
    {
                ...
                (args) =>
                {
                    if (args == null || args.Length == 0)
                    {
                        if (CurrentOrderInfo != null)
                        {
                            ...
                            Helpers.Blips.Core.DestroyTrackerBlipByKey("Taxi");
                        }
                    }
                    else if (args.Length == 2)
                    {
                        ...
                        Notification.Show(Notification.Types.Information,
                            ...
                            string.Format(Locale.Notifications.General.Taxi0, CurrentOrderInfo.Driver.Name)
                        );
                    }
                        ...
                        if (args[0] is bool b)
                        {
                            ...
                            if (!b)
                            {
                                ...
                                Notification.Show(Notification.Types.Error,
                                    Locale.Get("NOTIFICATION_HEADER_DEF"),
                                    string.Format(Locale.Notifications.General.Taxi1, CurrentOrderInfo.Driver.Name)
                                );
                            ...
                            else
                            {
                                ...
                                Notification.Show(Notification.Types.Information,
                                    Locale.Get("NOTIFICATION_HEADER_DEF"),
                                    string.Format(Locale.Notifications.General.Taxi2, Input.Core.Get(BindTypes.SendCoordsToDriver).GetKeyString())
                                );
                        ...
                        if (id == 0)
                        {
                            ...
                            if (res)
                            {
                                ...
                                Notification.Show(Notification.Types.Success,
                                    Locale.Get("NOTIFICATION_HEADER_DEF"),
                                    string.Format(Locale.Notifications.General.TaxiOrdered, Utils.Game.Misc.GetStreetName(pos))
                                );
                                ...
                                CurrentOrderInfo = new ClientOrderInfo()
                                {
                                    ...
                                    ExitColshape1 = new Cylinder(pos,
                                        Client.Settings.App.Static.TAXI_ORDER_MAX_WAIT_RANGE / 2,
                                        10f,
                                        false,
                                        Utils.Misc.RedColor,
                                        Client.Settings.App.Static.MainDimension
                                    )
                                    {
                                        OnExit = (cancel) =>
                                        {
                                            ...
                                            Notification.Show(Notification.Types.Information,
                                                Locale.Get("NOTIFICATION_HEADER_DEF"),
                                                string.Format(Locale.Notifications.General.TaxiDistanceWarn, Client.Settings.App.Static.TAXI_ORDER_MAX_WAIT_RANGE / 2)
                                            );
                                        },
                                    },
                                    ExitColshape2 = new Cylinder(pos,
                                        Client.Settings.App.Static.TAXI_ORDER_MAX_WAIT_RANGE,
                                        10f,
                                        false,
                                        new Utils.Colour(0, 0, 255, 25),
                                        Client.Settings.App.Static.MainDimension
                                    )
                                    {
                                        OnExit = (cancel) =>
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (id == 0)
                            {
                                ...
                                object chatList = SMS.GetChatList(allSms, CurrentOrderInfo.DriverNumber, pNumber);
        ...
        public static void Show(PlayerData pData)
        {
            ...
            if (CurrentOrderInfo == null)
            {
                Browser.Window.ExecuteJs("Phone.drawCabApp",
                    0,
                    new object[]
                    {
                        pData.CID.ToString(),
                        Utils.Game.Misc.GetStreetName(Player.LocalPlayer.Position),
                    }
                );
            }
            else
            {
                if (CurrentOrderInfo.Driver == null)
                    Browser.Window.ExecuteJs("Phone.drawCabApp",
                        ...
                        {
                            "Идет поиск водителя",
                            CurrentOrderInfo.Date.ToString("HH:mm dd.MM.yyyy"),
                        }
                    );
                else
                    Browser.Window.ExecuteJs("Phone.drawCabApp",
                        ...
                        {
                            "Водитель в пути",
                            CurrentOrderInfo.Driver.GetSharedData("CID", 0).ToString(),
                        }
                    );
            }
        }
