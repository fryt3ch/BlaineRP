...
namespace BlaineRP.Client.Game.UI.CEF
{
    ...
    public class Auth
    {
        ...
        public Auth()
        {
            Events.Add("Auth::Start::Show",
                async (args) =>
                {
                    var authData = (JObject)args[0];

                    byte authDataType = authData["Type"].ToObject<byte>();
                    string socialClubName = authData["SCName"].ToObject<string>();
                    ...
                    if (authDataType == 0)
                    {
                        ...
                        string login = RageStorage.GetData<string>("Auth::Login");
                        string token = RageStorage.GetData<string>("Auth::Token");
            ...
            Events.Add("Auth::CharSelect::Show",
                async (args) =>
                {
                    if ((bool)args[0])
                    {
                        ...
                        DateTime regDate = DateTimeOffset.FromUnixTimeSeconds(Utils.Convert.ToInt64(args[2])).DateTime;
                        var bCoins = Utils.Convert.ToDecimal(args[3]);

                        object[] data = ((JArray)args[4]).ToObject<object[]>();
                        ...
                        {
                            object[] cData = ((JArray)data[i])?.ToObject<object[]>();
                        ...
                        int? lastCharIdx = RageStorage.GetData<int?>("Auth::LastCharacter");
            ...
            Events.Add("Auth::RegistrationAttempt",
                (args) =>
                {
                    ...
                    string mail = ((string)args[1]).ToLower();
                    var pass1 = (string)args[2];
                    var pass2 = (string)args[3];

                    if (login.Length < 6 || login.Length > 12)
                    {
                        Notification.ShowError(login.Length > 12 ? Locale.Notifications.Auth.LoginTooLong : Locale.Notifications.Auth.LoginTooShort);
                    ...
                    {
                        Notification.ShowError(Locale.Notifications.Auth.InvalidLogin);
                    ...
                    {
                        Notification.ShowError(Locale.Notifications.Auth.InvalidMail);
                    ...
                    {
                        Notification.ShowError(pass1.Length > 64 ? Locale.Notifications.Auth.PasswordTooLong : Locale.Notifications.Auth.PasswordTooShort);
                    ...
                    {
                        Notification.ShowError(Locale.Notifications.Auth.InvalidPassword);
                    ...
                    {
                        Notification.ShowError(Locale.Notifications.Auth.PassNotMatch);

                        return;
                    }

                    Events.CallRemote("Auth::OnRegistrationAttempt", login, mail, pass1);
                }
            );

            Events.Add("Auth::LoginAttempt",
                (args) =>
                {
                    if (LastTime.IsSpam(500, false, true))
                        return;
                    ...
                    Events.CallRemote("Auth::OnLoginAttempt", login, pass, pass == RageStorage.GetData<string>("Auth::Token"));
                }
            );

            Events.Add("Auth::CharacterChooseAttempt",
                async (args) =>
                {
                    if (LastTime.IsSpam(500, false, false))
                        return;

                    int charNum = (int)args[0] - 1;

                    LastTime = Core.ServerTime;

                    var res = (int)await Events.CallRemoteProc("Auth::OnCharacterChooseAttempt", (byte)charNum);
