...
namespace BlaineRP.Client.Game.UI.CEF
{
    ...
    public class Menu
    {
        ...
        public Menu()
        {
            ...
            Events.Add("Menu::UpdateAimColor",
                (object[] args) =>
                {
            ...
            Events.Add("Menu::UpdateKeyBind",
                (object[] args) =>
                {
                    ...
                    var list = new List<RAGE.Ui.VirtualKeys>();
                    ...
                    Input.Core.Get(id)?.ChangeKeys(list.ToArray());
                }
            );
            ...
            Events.Add("Menu::Gifts::Update",
                (object[] args) =>
                {
                    ...
                    if (add)
                    {
                        var notify = (bool)args[2];

                        var type = (int)args[3];
                        var gid = (string)args[4];
                        var amount = (int)args[5];
                        var reason = (GiftSourceTypes)(int)args[6];

                        string name = GetGiftName((GiftTypes)type, gid, amount);

                        Gifts.Add(id, (Locale.Notifications.Gifts.SourceNames[reason], GetGiftName((GiftTypes)type, gid, amount)));

                        Notification.Show(Notification.Types.Gift,
                            Locale.Notifications.Gifts.Header,
                            string.Format(Locale.Notifications.Gifts.Added, name, Input.Core.Get(BindTypes.Menu).GetKeyString())
                        );
                    }
                    else
                    {
                    ...
                    Browser.Window.ExecuteJs("Menu.drawGifts",
                        new object[]
                        {
                            Gifts.Select(x => new object[]
                                {
                                    x.Key,
                                    x.Value.Reason,
                                    x.Value.Name,
            ...
            Events.Add("Menu::Quests::Locate",
                (args) =>
                {
                    ...
                    List<Quest> quests = pData.Quests;

                    Quest quest = quests.Where(x => x.Type == (QuestTypes)Enum.Parse(typeof(QuestTypes), (string)args[0])).FirstOrDefault();
        ...
        public static bool IsActive => Browser.IsActive(Browser.IntTypes.Menu);
        ...
        public static TimeSpan TimePlayed
        {
            ...
            {
                _TimePlayed = value;
                Browser.Window.ExecuteJs("Menu.setPlayed", value.TotalHours.ToString("0.0"));
            }
        }
        ...
        {
            Browser.Window.ExecuteJs("Menu.selectOption", "menu-char");

            Browser.Window.ExecuteJs("Menu.drawSkills",
                new object[]
                {
                    Settings.App.Static.PlayerMaxSkills.Select(x => new object[]
                        {
                            x.Key,
                            x.Value,
                        }
            ...
            Browser.Window.ExecuteJs("Menu.createManyToggles",
                "main",
                mainDict.Select(x => new object[]
                    {
                        x.Key,
                        x.Value,
                    }
        ...
        public static void Show(SectionTypes sType = SectionTypes.Last)
        {
            ...
            TempBindEsc = Input.Core.Bind(RAGE.Ui.VirtualKeys.Escape, true, () => Close());
        ...
        public static string GetGiftName(GiftTypes type, string gid, int amount)
        {
            if (type == GiftTypes.Item)
                return Items.Core.GetName(gid) + (amount > 1 ? $" x{amount}" : "");
        ...
        {
            Browser.Window.ExecuteJs("Menu.setCash", value);
        }

        public static void SetBank(ulong value)
        {
            Browser.Window.ExecuteJs("Menu.setBank", value);
        }

        public static void SetSex(bool state)
        {
            Browser.Window.ExecuteJs("Menu.setSex", state);
        }

        public static void SetName(string value)
        {
            Browser.Window.ExecuteJs("Menu.setName", value);
        }

        public static void SetCID(uint value)
        {
            Browser.Window.ExecuteJs("Menu.setCID", value);
        }

        public static void SetFraction(FractionTypes type)
        {
            Browser.Window.ExecuteJs("Menu.setFraction", Fraction.Get(type)?.Name ?? Fraction.NoFractionStr);
        }

        public static void SetOrganisation(string name)
        {
            Browser.Window.ExecuteJs("Menu.setOrganisation", name ?? "null");
        }

        public static void UpdateSkill(SkillTypes type, int current)
        {
            Browser.Window.ExecuteJs("Menu.setSkill", type, current);
        }

        public static void UpdateAchievement(AchievementTypes aType, int current, int max)
        {
            Browser.Window.ExecuteJs("Menu.updateAchProgress", aType.ToString(), current, max);
        }

        public static void AddAchievement(AchievementTypes aType, int current, int max, string name, string desc)
        {
            Browser.Window.ExecuteJs("Menu.newAchievement",
                new object[]
                {
                    new object[]
                    {
        ...
        {
            Browser.Window.ExecuteJs("Menu.updateQuestProgress");
            // id, new_progress
        }

        public static void Load(PlayerData pData,
                                TimeSpan timePlayed,
                                DateTime creationDate,
                                DateTime birthDate,
                                Dictionary<uint, (int Type, string GID, int Amount, int Reason)> gifts)
        {
            ...
            if (gifts.Count > 0)
            {
                foreach (KeyValuePair<uint, (int Type, string GID, int Amount, int Reason)> x in gifts)
                {
                    Gifts.Add(x.Key,
                        (Locale.Notifications.Gifts.SourceNames.GetValueOrDefault((GiftSourceTypes)x.Value.Reason) ?? "null",
                            GetGiftName((GiftTypes)x.Value.Type, x.Value.GID, x.Value.Amount))
                    );
                }
        ...
        public static void UpdateProperties(PlayerData pData = null)
        {
            ...
            properties.AddRange(pData.OwnedVehicles.Select(x => new object[]
                    {
                        "veh",
            ...
            properties.AddRange(pData.OwnedBusinesses.Select(x => new object[]
                    {
                        "est",
            ...
            properties.AddRange(pData.OwnedHouses.Select(x => new object[]
                    {
                        "est",
            ...
            properties.AddRange(pData.OwnedApartments.Select(x => new object[]
                    {
                        "est",
            ...
            properties.AddRange(pData.OwnedGarages.Select(x => new object[]
                    {
                        "est",
            ...
            Browser.Window.ExecuteJs("Menu.fillProperties",
                new object[]
                {
                    properties,
                }
        ...
        public static void UpdateQuests(PlayerData pData = null)
        {
            ...
            Browser.Window.ExecuteJs("Menu.drawQuests",
                new object[]
                {
                    quests.Select(x => new object[]
                        {
                            x.Type.ToString(),
                            x.Data.Name,
                            x.Data.GiverName,
                            x.GoalWithProgress ?? "null",
                            (int)x.Data.ColourType,
                        }
                    ),
                }
            );
        ...
        public static void AddMessageToChatHistory(Player player, DateTime time, string text)
        {
            if (player == Player.LocalPlayer)
                ...
                Browser.Window.ExecuteJs("Menu.newHelpMessage",
                    true,
                    time.ToString("HH:mm"),
                    $"{player.Name} [#{Utils.Convert.ToDecimal(player.GetSharedData<object>("CID", 0))}]",
                    text
                );
        }
        ...
        {
            List<object[]> list = Input.Core.Binds.Where(x => x.Value.Changeable && !x.Value.InvOnly)
