...
namespace BlaineRP.Client.Game.UI.CEF
{
    ...
    public class FractionMenu
    {
        ...
        public FractionMenu()
        {
            ...
            Events.Add("MenuFrac::Veh",
                async (args) =>
                {
                    ...
                    if (actionId == 0) // gps
                    {
                        ...
                        Helpers.Blips.Core.CreateGPS(coords, Settings.App.Static.MainDimension, true);
                    }
                    else if (actionId == 1) // respawn
                    {
                        ...
                        Fractions.VehicleData vData = Fraction.AllVehicles.GetValueOrDefault(vid);
                        ...
                        if ((bool)await Events.CallRemoteProc("Fraction::VRSP", vid))
                            Notification.Show(Notification.Types.Success,
                                ...
                                $"Транспорт {vData.Numberplate} был возвращен на свое парковочное место!"
                            );
                    }
                }
            );

            Events.Add("MenuFrac::ChangeVehAccess",
                async (args) =>
                {
                    ...
                    {
                        Notification.Show("Fraction::NAL");
                    ...
                    {
                        Notification.ShowError($"Этот ранг уже установлен в качестве минимального для данного транспорта!");
                    ...
                    if ((bool)await Events.CallRemoteProc("Fraction::VCMR", vid, newMinRank))
                        Notification.Show(Notification.Types.Success,
                            Locale.Get("NOTIFICATION_HEADER_DEF"),
                            $"Вы изменили минимальный ранг для доступа к транспорту {vData.Numberplate} на {newMinRank + 1} - {fData.GetRankName(newMinRank)}!"
                        );
                }
            );

            Events.Add("MenuFrac::EmployeeAction",
                async (args) =>
                {
                    ...
                    if (actionId == 0) // rank up
                    {
                        ...
                        {
                            Notification.ShowError("Этот сотрудник уже имеет максимально возможную должность!");
                    ...
                    else if (actionId == 1) // rank down
                    {
                        if (mData.Rank <= 0)
                        {
                            Notification.ShowError("Этот сотрудник уже имеет минимально возможную должность!");
                        ...
                        if ((bool)await Events.CallRemoteProc("Fraction::MRC", cid, newRank))
                            Notification.Show(Notification.Types.Success,
                                Locale.Get("NOTIFICATION_HEADER_DEF"),
                                $"Вы понизили сотрудника {mData.Name} #{cid}!\nЕго новая должность - {newRank + 1} - {fData.GetRankName(newRank)}"
                            );
                    }
                    else if (actionId == 2) // fire
                    {
                        ...
                        if ((bool)await Events.CallRemoteProc("Fraction::MF", cid))
                            Notification.Show(Notification.Types.Success, Locale.Get("NOTIFICATION_HEADER_DEF"), $"Вы уволили сотрудника {mData.Name} #{cid}!");
                    }
                }
            );

            Events.Add("MenuFrac::EditPosition",
                async (args) =>
                {
                    var pData = PlayerData.GetData(Player.LocalPlayer);
                    ...
                    Dictionary<uint, int> rankData = ((JObject)await Events.CallRemoteProc("Fraction::RE", rankToEdit))?.ToObject<Dictionary<uint, int>>();
                    ...
                    Browser.Window.ExecuteJs("MenuFrac.showEdit",
                        true,
                        new object[]
                        {
                            rankToEdit + 1,
                            rankData.Select(x => new object[]
                                {
                                    x.Key,
                                    Fraction.GetFractionPermissionName(x.Key) ?? "null",
                                    x.Value == 1,
                                }
                            ),
                        }
            ...
            Events.Add("MenuFrac::AccessButtons",
                async (args) =>
                {
                    var pData = PlayerData.GetData(Player.LocalPlayer);
                    ...
                    if (id == "storage")
                    {
                        if ((bool)await Events.CallRemoteProc("Fraction::SL", state))
                            Notification.Show(Notification.Types.Success, Locale.Get("NOTIFICATION_HEADER_DEF"), state ? "Вы закрыли склад!" : "Вы открыли склад!");
                    }
                    else if (id == "workbench")
                    {
                        if ((bool)await Events.CallRemoteProc("Fraction::CWBL", state))
                            Notification.Show(Notification.Types.Success,
                                Locale.Get("NOTIFICATION_HEADER_DEF"),
                                state ? "Вы закрыли создание предметов из материалов!" : "Вы открыли создание предметов из материалов!"
                            );
                    }
                }
            );

            Events.Add("MenuFrac::NewsAction",
                async (args) =>
                {
                    var pData = PlayerData.GetData(Player.LocalPlayer);
                    ...
                    if (actionId == 0) // add news
                    {
                        ...
                        Browser.Window.ExecuteJs("MenuFrac.editNews", true, null, null);
                    }
                    else if (actionId == 2) // cancel edit/add
                    {
                        Player.LocalPlayer.ResetData("MenuFrac::News::Edit::CurrentId");

                        Browser.Window.ExecuteJs("MenuFrac.editNews", false);
                    }
                    else if (actionId == 1) // add/edit news
                    {
                        ...
                        string text = ((string)args[2])?.Trim();
                        ...
                        {
                            Notification.ShowError(string.Format(Locale.Notifications.General.MaximalNewLineCharacterCount, NEWS_TEXT_MAX_NL), -1);
                        ...
                        {
                            int editIdx = Player.LocalPlayer.GetData<int>("MenuFrac::News::Edit::CurrentId");
            ...
            Events.Add("MenuFrac::Tooltip",
                async (args) =>
                {
                    ...
                    if (tooltipId.Contains("news"))
                    {
                        ...
                        {
                            string text = Fraction.NewsData.All.GetValueOrDefault(idx);
                            ...
                            Browser.Window.ExecuteJs("MenuFrac.editNews", true, idx, text);
                        }
            ...
            Events.Add("MenuFrac::SetPermit",
                async (args) =>
                {
                    ...
                    if ((bool)await Events.CallRemoteProc("Fraction::RUP", Player.LocalPlayer.GetData<byte>("FractionMenu::RankEdit::CurrentRank"), permId, state))
                    {
                        ...
                        Browser.Window.ExecuteJs("MenuFrac.setPermit", permId, state);
                    }
                }
            );

            Events.Add("MenuFrac::PositionName",
                async (args) =>
                {
                    ...
                    {
                        Notification.ShowError("Название ранга должно быть от 2х до 12ти букв, цифр или знака пробел!");
                    ...
                    byte rankToEdit = Player.LocalPlayer.GetData<byte>("FractionMenu::RankEdit::CurrentRank");

                    Fraction fData = PlayerData.GetData(Player.LocalPlayer)?.CurrentFraction;
                    ...
                    {
                        Notification.ShowError("Эта должность уже имеет такое название!");
                    ...
                    for (byte i = 0; i <= fData.MaxRank; i++)
                    {
                        if (fData.GetRankName(i) == name)
                        {
                            if (i == rankToEdit)
                                ...
                                Notification.ShowError("Одна из должностей уже имеет такое название!");
                    ...
                    if ((bool)await Events.CallRemoteProc("Fraction::RUN", rankToEdit, name))
                        Notification.Show(Notification.Types.Success, Locale.Get("NOTIFICATION_HEADER_DEF"), $"Вы изменили название должности {rankToEdit + 1} на \"{name}\"!");
                }
            );
        }

        public static bool IsActive => Browser.IsActive(Browser.IntTypes.MenuFraction);

        private static int EscBindIdx { get; set; } = -1;

        public static async void Show(FractionTypes type,
                                      NewsData newsData,
                                      Dictionary<uint, MemberData> members,
                                      Dictionary<uint, Fractions.VehicleData> vehs,
                                      decimal balance,
                                      byte myRank)
        {
            if (IsActive)
            ...
            await Browser.Render(Browser.IntTypes.MenuFraction, true, true);

            Cursor.Show(true, true);

            EscBindIdx = Core.Bind(RAGE.Ui.VirtualKeys.Escape, true, () => Close());

            Browser.Window.ExecuteJs("MenuFrac.draw",
                new List<object>()
                {
                    new object[]
                    {
                        new object[]
                        {
                            newsData.All.Select(x => new object[]
                                {
                                    x.Key,
                                    x.Value,
                                }
                            ),
                            newsData.PinnedId < 0 ? (int?)null : newsData.PinnedId,
                        },
                        new object[]
                        {
                            fData.Name,
                            members.GetValueOrDefault(fData.LeaderCID)?.Name ?? "Отсутствует",
                            balance,
                            fData.Materials,
                            myRank + 1,
                        },
                    },
                    members.Select(x => new object[]
                        {
                            x.Value.IsOnline,
                            x.Value.SubStatus,
                            x.Value.Name,
                            x.Key,
                            x.Value.LastSeenDate.ToString("dd.MM.yyyy HH:mm"),
                            x.Value.Rank + 1,
                        }
                    ),
                    new object[]
                    {
                        Enumerable.Range(0, fData.MaxRank + 1)
                                  .Select(x => new object[]
                                       {
                                           x + 1,
                                           fData.GetRankName((byte)x),
                                       }
                                   ),
                        vehs.Select(x => new object[]
                            {
                                x.Key,
                                x.Value.Numberplate,
                                x.Value.MinRank + 1,
        ...
        public static void Close()
        {
            ...
            Browser.Render(Browser.IntTypes.MenuFraction, false);
        ...
        public static void SetLockButtonState(string id, bool state)
        {
            ...
            Browser.Window.ExecuteJs("MenuFrac.setButton", id, state);
        }
        ...
        public static void UpdateVehicleInfo(uint vid, string where, object value)
        {
            ...
            Browser.Window.ExecuteJs("MenuFrac.updateVeh", vid, where, value);
        }
        ...
        public static void RemoveMember(uint cid)
        {
            ...
            Browser.Window.ExecuteJs("MenuFrac.deleteEmployee", cid);
        }
        ...
        public static void UpdateMember(uint cid, string where, object value)
        {
            ...
            Browser.Window.ExecuteJs("MenuFrac.updateEmployee", cid, where, value);
        }
        ...
        public static void AddMember(uint cid, MemberData mData)
        {
            ...
            Browser.Window.ExecuteJs("MenuFrac.addEmployee", mData.IsOnline, mData.SubStatus, mData.Name, cid, mData.LastSeenDate.ToString("dd.MM.yyyy HH:mm"), mData.Rank + 1);
        }
        ...
        public static void UpdateRankName(byte rank, string name)
        {
            ...
            Browser.Window.ExecuteJs("MenuFrac.updatePositionName", rank, name);
        }
        ...
        public static void UpdateInfoLine(int num, object value)
        {
            ...
            Browser.Window.ExecuteJs("MenuFrac.updateInfoLine", num, value);
        }
        ...
        public static void AddNews(int idx, string text)
        {
            ...
            Browser.Window.ExecuteJs("MenuFrac.addNews", idx, text);
        }
        ...
        public static void UpdateNews(int idx, string text)
        {
            ...
            Browser.Window.ExecuteJs("MenuFrac.updateNews", idx, text);
        }
        ...
        public static void DeleteNews(int idx)
        {
            ...
            Browser.Window.ExecuteJs("MenuFrac.deleteNews", idx);
        }
        ...
        public static void PinNews(int idx)
        {
            ...
            if (idx < 0)
                Browser.Window.ExecuteJs("MenuFrac.unpinNews();");
            else
                Browser.Window.ExecuteJs("MenuFrac.pinNews", idx);
        }
    }
}