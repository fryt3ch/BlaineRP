...
namespace BlaineRP.Client.Game.UI.CEF
{
    ...
    public class Audio
    {
        ...
        private static string[] DefaultStreamFormats = new string[]
        {
            "mp3",
        };
        ...
        public Audio()
        {
                ...
                async (args) =>
                {
                    ...
                    if (audioData == null)
                    {
                        if (aId.StartsWith("ONCE_"))
                            Browser.Window.ExecuteJs("stopAudio", aId);
                    ...
                    if (audioData.Id == "AUTH_AUDIO_PL")
                    {
                        ...
                        int nextTrackIdx = authAudios.IndexOf(audioData.CurrentTrackType) + 1;
            ...
            Events.Add("Audio::Loaded",
                (args) =>
                {
                    ...
                    float actualPos = audioData.GetActualPosSeconds();
            ...
            new AsyncTask(() =>
                {
                    float userVolume = Settings.User.Audio.SoundVolume / 100f;

                    if (!Utils.Misc.IsGameWindowFocused && Settings.User.Native.Audio_MuteAudioOnFocusLoss)
                        userVolume = 0f;

                    for (var i = 0; i < AllAudios.Count; i++)
                    {
                        Data audio = AllAudios[i];
                        ...
                        {
                            float dist = Player.LocalPlayer.Position.DistanceTo(audio.Position);
            ...
            Events.AddDataHandler("AUD",
                (entity, value, oldValue) =>
                {
                    if (entity is GameEntity gEntity)
                    {
                        ...
                        string audioId = GetEntityAudioId(gEntity, "D");

                        Data currentAudio = AllAudios.Where(x => x.Id == audioId).FirstOrDefault();
        ...
        public static bool IsTrackStream(TrackTypes trackType)
        {
            return trackType >= TrackTypes.RadioRetroFM;
        }
        ...
        public static void UpdateAudioState(Data audio, float dist, float userVolume)
        {
            ...
            {
                float newVolume = (audio.BaseVolume - dist / audio.Range * audio.BaseVolume) * userVolume;
        ...
        public static void StartAuthPlaylist()
        {
            ...
            audioData.Play(authAudios[Utils.Misc.Random.Next(0, authAudios.Count)], audioData.BaseVolume, false, 0);
        }
        ...
        {
            string[] audioData = gEntity.GetSharedData<string>("AUD")?.Split('&');
        ...
        {
            List<Data> allAudio = Data.GetAllByEntity(gEntity);

            foreach (Data x in allAudio)
            {
                x.Destroy();
            }
        }
        ...
        public static string GetEntityAudioId(GameEntity gEntity, string tag)
        {
            return $"ENT_{tag}_{gEntity.Handle}";
        }
        ...
        public static void PlayOnce(string tag, TrackTypes trackType, float volume = 1f, int pos = 0)
        {
            ...
            string url = Urls.GetValueOrDefault(trackType);
        ...
        public class Data
        {
            ...
            public static Data GetById(string id)
            {
                return AllAudios.Where(x => x.Id == id).FirstOrDefault();
            }

            public static List<Data> GetAllByEntity(GameEntity gEntity)
            {
                return AllAudios.Where(x => x.Entity == gEntity).ToList();
            }
            ...
            public float GetActualPosSeconds()
            {
                ...
                return (float)Core.ServerTime.Subtract(StartTime).TotalSeconds;
            }
        }
    }
}