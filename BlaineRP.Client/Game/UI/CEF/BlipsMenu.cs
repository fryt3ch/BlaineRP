...
namespace BlaineRP.Client.Game.UI.CEF
{
    ...
    public class BlipsMenu
    {
        public BlipsMenu()
        {
            ...
            Events.Add("BlipMenu::Local::Create",
                (object[] args) =>
                {
                    ...
                    List<LocalBlip> allBlips = Settings.User.Other.LocalBlips;

                    var blip = new LocalBlip(CurrentSprite,
                        name,
                        CurrentColour,
                        CurrentScale,
                        CurrentAlpha,
                        CurrentUsePos ? Player.LocalPlayer.Position : Main.WaypointPosition ?? Player.LocalPlayer.Position,
                        false,
                        true
                    );
                    ...
                    Browser.Window.ExecuteJs("Blips.newBlip", blip.Name, blip.Enabled, blip.Colour, blip.Sprite, blip.Scale, blip.Alpha, !blip.ShortRange);
                }
            );

            Events.Add("BlipMenu::Local::Edit",
                (object[] args) =>
                {
                    var idx = (int)args[0];
                    ...
                    Browser.Window.ExecuteJs("Blips.editBlip", idx, blip.Name, blip.Colour, blip.Sprite, blip.Scale, blip.Alpha, !blip.ShortRange);
                }
            );

            Events.Add("BlipMenu::Local::Delete",
                (object[] args) =>
                {
                    var idx = (int)args[0];
                    ...
                    Browser.Window.ExecuteJs("Blips.removeBlip", idx);
            ...
            Events.Add("BlipMenu::Local::Toggle",
                (object[] args) =>
                {
                    ...
                    Browser.Window.ExecuteJs("Blips.setCheckBoxState", (int)args[2], state);
                }
            );

            Events.Add("BlipMenu::Local::SetProperty",
                (object[] args) =>
                {
                    if (!IsActive)
                        return;
                    ...
                    if (idx < 0)
                    {
                        if (LastEdited != -1)
                        {
                            LocalBlip blipT = Settings.User.Other.LocalBlips[LastEdited];
                    ...
                    var blip = new ExtraBlip((uint)CurrentSprite,
                        TempBlip.Position,
                        "",
                        CurrentScale,
                        CurrentColour,
                        (int)System.Math.Floor(CurrentAlpha * 255),
                        0f,
                        TempBlip.IsShortRange,
                        0,
                        0f,
                        uint.MaxValue
                    );
        ...
        public static bool IsActive => Browser.IsActive(Browser.IntTypes.BlipsMenu);
        ...
        public static void Show()
        {
            ...
            Browser.Switch(Browser.IntTypes.BlipsMenu, true);

            if (FirstOpen)
            {
                IEnumerable<object[]> data = Settings.User.Other.LocalBlips.Select(x => new object[]
                    {
                        x.Name,
                        x.Enabled,
                        x.Colour,
                ...
                Browser.Window.ExecuteJs("Blips.fillBlips",
                    new object[]
                    {
                        data,
                    }
            ...
            if (WasCreating)
                TempBlip = new ExtraBlip((uint)CurrentSprite,
                    CurrentUsePos ? Player.LocalPlayer.Position : Main.WaypointPosition ?? Player.LocalPlayer.Position,
                    "",
                    CurrentScale,
                    CurrentColour,
                ...
                TempBlip = new ExtraBlip((uint)CurrentSprite,
                    Settings.User.Other.LocalBlips[LastEdited].Position,
                    "",
                    CurrentScale,
                    CurrentColour,
                    (int)System.Math.Floor(CurrentAlpha * 255),
            ...
            int playerBlip = RAGE.Game.Ui.GetMainPlayerBlipId();
            int waypointBlip = Utils.Game.Misc.GetWaypointBlip();
        ...
        public static void Close(bool ignoreTimeout = false)
        {
            ...
            Browser.Switch(Browser.IntTypes.BlipsMenu, false);
