...
/*            { Types.FishingShop, "fish" },
        ...
        public Shop()
        {
            ...
            Events.Add("Shop::TestDrive", async (object[] args) =>
            {
                if (CurrentType >= Types.CarShop1 && CurrentType <= Types.AeroShop)
                {
                    ...
                    TempBinds.Add(Core.Bind(RAGE.Ui.VirtualKeys.F4, true, () => TuningMenu.Show()));
                }
            });
            ...
            Events.Add("Shop::Choose", async (object[] args) =>
            {
                ...
                else if (CurrentType == Types.TuningShop)
                {
                    ...
                    {
                        var t = TuningMenu.Slots.Where(x => x.Value.Id == data[0]).FirstOrDefault();
            ...
            if (heading != null)
            {
                ...
                task = new AsyncTask(async () =>
                {
                    ...
                    else if (type == Types.TuningShop)
                    {
                        ...
                        foreach (var x in TuningMenu.Slots.Where(x => x.Value.Id == "spoiler" || x.Value.Id == "fbump" || x.Value.Id == "rbump" || x.Value.Id == "skirt" || x.Value.Id == "exh" || x.Value.Id == "frame" || x.Value.Id == "grill" || x.Value.Id == "hood" || x.Value.Id == "roof" || x.Value.Id == "seats" || x.Value.Id == "swheel" || x.Value.Id == "livery" || x.Value.Id == "horn").ToDictionary(x => x.Value.Id, x => (x.Value.Name, x.Key)))
                        {
                        ...
                        if (vData.Data.Type != Game.Data.Vehicles.Types.Boat)
                        {
                            ...
                            if (vData.Data.Type == Game.Data.Vehicles.Types.Motorcycle)
                            {
                                ...
                                subData.Add(new object[] { $"wheel_0", TuningMenu.WheelTypes[0], "variants-list", new object[] { prices[$"wheel_0"] }, currentWheel < 0 ? $"wheel_0_0" : null });

                                var wAmount = veh.GetNumMods(23);

                                if (wAmount > 0)
                                    subData.Add(new object[] { $"wheel_7", TuningMenu.WheelTypes[7], "variants-list", Enumerable.Range(0, wAmount).Select(y => prices[$"wheel_7"]), currentWheel >= 0 ? $"wheel_0_{currentWheel}" : null });
                                ...
                                subData.Add(new object[] { $"rwheel_0", TuningMenu.WheelTypes[0], "variants-list", new object[] { prices[$"rwheel_0"] }, currentWheel < 0 ? $"rwheel_0_0" : null });

                                wAmount = veh.GetNumMods(24);

                                if (wAmount > 0)
                                    subData.Add(new object[] { $"rwheel_7", TuningMenu.WheelTypes[7], "variants-list", Enumerable.Range(0, wAmount).Select(y => prices[$"rwheel_7"]), currentWheel >= 0 ? $"rwheel_0_{currentWheel}" : null });
                            ...
                            else
                            {
                                ...
                                foreach (var x in TuningMenu.WheelTypes)
                                {
                                    if (x.Key == 7)
                                        continue;
        ...
        private static async void StopTestDrive()
        {
            ...
            Game.Management.AntiCheat.Core.LastPosition = DefaultPosition;

            Player.LocalPlayer.Position = Game.Management.AntiCheat.Core.LastPosition;
