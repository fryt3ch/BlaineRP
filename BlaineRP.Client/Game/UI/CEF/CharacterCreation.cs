...
namespace BlaineRP.Client.Game.UI.CEF
{
    ...
    public class CharacterCreation
    {
        ...
        private static Dictionary<int, Customization.HeadOverlay> _headOverlays;
        private static float[] _faceFeatures;
        private static Customization.HairStyle _hairStyle;
        ...
        {
            Events.Add("CharacterCreation::StartNew",
                async (args) =>
                {
                    await StartNew();
                }
            );
            Events.Add("CharacterCreation::Close",
                async (args) =>
                {
                    await Close();
                }
            ...
            Events.Add("CharacterCreation::Create",
                async (args) =>
                {
                    ...
                    {
                        Notification.ShowError(Locale.Notifications.CharacterCreation.WrongName);
                    ...
                    {
                        Notification.ShowError(Locale.Notifications.CharacterCreation.WrongSurname);
                    ...
                    {
                        Notification.ShowError(Locale.Notifications.CharacterCreation.WrongAge);
                    ...
                    if (Notification.HasApproveTimedOut(approveContext, World.Core.ServerTime, approveTime))
                    {
                        ...
                        Notification.Show(Notification.Types.Question,
                            ...
                            Locale.Notifications.CharacterCreation.PressAgainToCreate,
                            approveTime
                        );
                    }
                    else
                    {
                        ...
                        object res = await Events.CallRemoteProc("CharacterCreation::Create",
                            name,
                            surname,
                            age,
                            _sex,
                            _eyeColor,
                            RAGE.Util.Json.Serialize(_headBlend),
                            RAGE.Util.Json.Serialize(_headOverlays),
                            RAGE.Util.Json.Serialize(_hairStyle),
                            RAGE.Util.Json.Serialize(_faceFeatures),
                            RAGE.Util.Json.Serialize(_clothes)
                        );
                    }
                }
            );

            Events.Add("CharacterCreation::OnExit",
                (object[] args) =>
                {
                    ...
                    if (Notification.HasApproveTimedOut(approveContext, World.Core.ServerTime, approveTime))
                    {
                        ...
                        Notification.Show(Notification.Types.Question,
                            Locale.Get("NOTIFICATION_HEADER_APPROVE"),
                            Locale.Notifications.CharacterCreation.PressAgainToExit,
                            approveTime
                        );
                    }
                    else
            ...
            Events.Add("CharacterCreation::SetSex",
                async (args) =>
                {
            ...
            Events.Add("CharacterCreation::GetRandomName",
                (object[] args) =>
                {
                    if (!IsActive)
                        return;

                    Random r = Utils.Misc.Random;

                    Browser.Window.ExecuteJs("ChCreate.nameSet",
                        _sex ? _maleNames[r.Next(0, _maleNames.Length - 1)] : _femaleNames[r.Next(0, _maleNames.Length - 1)],
                        _surnames[r.Next(0, _surnames.Length - 1)]
                    );
                }
            );
            ...
            Events.Add("CharacterCreation::Update::Parents",
                (object[] args) =>
                {
                    ...
                    Player.LocalPlayer.SetHeadBlendData(_headBlend.GetMother(),
                        _headBlend.GetFather(),
                        0,
                        _headBlend.GetMother(),
                        _headBlend.GetFather(),
                        0,
                        _headBlend.ShapeMix,
                        _headBlend.SkinMix,
            ...
            Events.Add("CharacterCreation::Update::FaceFeature",
                (object[] args) =>
                {
                    var id = (int)args[1];
                    float value = RAGE.Util.Json.Deserialize<float>((string)args[0]);
            ...
            Events.Add("CharacterCreation::Update::HeadOverlay",
                (object[] args) =>
                {
                    var id = (int)args[1];
            ...
            Events.Add("CharacterCreation::Update::HeadOverlayOpacity",
                (object[] args) =>
                {
                    var id = (int)args[1];
                    float value = RAGE.Util.Json.Deserialize<float>((string)args[0]);
            ...
            Events.Add("CharacterCreation::Update::HeadOverlayColor",
                (object[] args) =>
                {
                    var id = (int)args[1];
                    var value = (byte)(int)args[0];
            ...
            Events.Add("CharacterCreation::Update::Hair",
                (object[] args) =>
                {
                    var id = (int)args[0];
                    int value = Customization.GetHair(_sex, id);
                    ...
                    if (IsActive)
                        Browser.Window.ExecuteJs("ChCreate.setHairFuzz", Customization.GetDefaultHairOverlayId(_sex, (int)args[0]));
                }
            );

            Events.Add("CharacterCreation::Update::HairOverlay",
                (object[] args) =>
                {
                    var value = (byte)(int)args[0];
                    ...
                    Customization.HairOverlay overlay = Customization.GetHairOverlay(_sex, value);
            ...
            Events.Add("CharacterCreation::Update::HairColor",
                (object[] args) =>
                {
                    var value = (byte)(int)args[0];
                    ...
                    if (Customization.GetHairOverlay(_sex, _hairStyle.Overlay) is Customization.HairOverlay overlay)
                    {
                        Player.LocalPlayer.ClearFacialDecorations();
            ...
            Events.Add("CharacterCreation::Update::EyeColor",
                (object[] args) =>
                {
                    var value = (byte)(int)args[0];
            ...
            Events.Add("CharacterCreation::Update::Clothes",
                (object[] args) =>
                {
                    var type = (int)args[1];
                    ...
                    if (value != -1)
                    {
                        ...
                        string clothes = _defaultClothes[_sex][type][value];

                        _clothes[type] = clothes;

                        Data.Customization.Clothes.Wear(clothes, 0);
                    }
                    else
                    {
                        _clothes[type] = null;
        ...
        private static void SetDefaultCustomization()
        {
            ...
            _headBlend.ShapeMix = _sex ? 0.5f : 0f;
            _headBlend.SkinMix = 0.5f;

            _eyeColor = 0;
            _hairStyle.Id = 0;
            _hairStyle.Overlay = 0;
            _hairStyle.Color = 0;
            _hairStyle.Color2 = 0;
        ...
        {
            foreach (int bind in _tempBinds)
            {
        ...
        public static async System.Threading.Tasks.Task StartNew()
        {
            ...
            _headBlend = new Customization.HeadBlend();
            _hairStyle = new Customization.HairStyle();
            _headOverlays = new Dictionary<int, Customization.HeadOverlay>();

            for (var i = 0; i < 13; i++)
            {
                _headOverlays.Add(i, new Customization.HeadOverlay());
            }
            ...
            Notification.ShowHint(Locale.Notifications.CharacterCreation.CtrlMovePed, true, 5000);
            ...
            _tempBinds.Add(Input.Core.Bind(RAGE.Ui.VirtualKeys.V,
                    true,
                    () =>
                    {
        ...
        public static void OnTickMouse()
        {
            ...
            float dist = curPos.Distance(_lastCursorPos);
            float newHeading = Player.LocalPlayer.GetHeading();
