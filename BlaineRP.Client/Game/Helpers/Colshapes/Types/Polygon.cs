...
namespace BlaineRP.Client.Game.Helpers.Colshapes.Types
{
    ...
    {
        protected Polygon(ColshapeTypes Type, List<Vector3> Vertices, float Height, float Heading, bool IsVisible, Colour Colour, uint Dimension, Colshape Colshape = null) : base(
            Type,
            IsVisible,
            Colour,
            Dimension,
        ...
        )
        {
            ...
            Position = GetCenterPosition(Vertices, Height);
        ...
        public Polygon(List<Vector3> Vertices, float Height, float Heading, bool IsVisible, Colour Colour, uint Dimension, Colshape Colshape = null) : this(ColshapeTypes.Polygon,
            Vertices,
            Height,
            Heading,
            IsVisible,
        ...
        public override void Draw()
        {
            ...
            int vertIdLimiter = Vertices.Count <= 50 ? 1 : 10;

            if (Vertices.Count == 1)
            {
                Vector3 vertice = Vertices[0];
            ...
            else if (Settings.User.Other.HighPolygonsMode)
            {
                if (Height == 0)
                    for (var i = 0; i < Vertices.Count; i++)
                    {
                        Vector3 currentVertice = Vertices[i];
                        Vector3 nextVertice = i == Vertices.Count - 1 ? Vertices[0] : Vertices[i + 1];

                        RAGE.Game.Graphics.DrawPoly(currentVertice.X,
                            currentVertice.Y,
                            currentVertice.Z,
                            nextVertice.X,
                            nextVertice.Y,
            ...
            else
            {
                if (Height == 0)
                    for (var i = 0; i < Vertices.Count; i++)
                    {
                        ...
                        RAGE.Game.Graphics.DrawLine(currentVertice.X,
                            currentVertice.Y,
                            currentVertice.Z,
                            nextVertice.X,
                            nextVertice.Y,
                    ...
                    for (var i = 0; i < Vertices.Count; i++)
                    {
                        ...
                        RAGE.Game.Graphics.DrawLine(nextVertice.X,
                            nextVertice.Y,
                            nextVertice.Z,
                            nextVertice.X,
                            nextVertice.Y,
            ...
            if (Settings.User.Other.DebugLabels)
            {
                ...
                Graphics.DrawText($"Vertices: {Vertices.Count} | Height: {Height}",
                    screenX,
        ...
        public override bool IsPointInside(Vector3 point)
        {
            ...
            for (var i = 0; i < Vertices.Count; i++)
            {
                ...
                var p2 = new Vector3(Vertices[(i + 1) % Vertices.Count].X - point.X,
                    Vertices[(i + 1) % Vertices.Count].Y - point.Y,
                    Vertices[(i + 1) % Vertices.Count].Z - point.Z
                );

                double m1 = System.Math.Sqrt(p1.X * p1.X + p1.Y * p1.Y + p1.Z * p1.Z);
                double m2 = System.Math.Sqrt(p2.X * p2.X + p2.Y * p2.Y + p2.Z * p2.Z);
