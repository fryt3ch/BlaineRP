...
namespace BlaineRP.Client.Game.Helpers.Blips
{
    ...
    public class Core
    {
        public Core()
        {
            ...
            Events.Add("Blip::CreateGPS",
                (args) =>
                {
                    var pos = (Vector3)args[0];
                    var dim = Convert.ToUInt32(args[1]);
            ...
            Events.Add("Blip::Tracker",
                (args) =>
                {
                    ...
                    AsyncTask task = entity?.Exists == true
                        ? new AsyncTask(() =>
                            {
                                if (!entity.Exists)
                                    return;
                                Vector3 coords = RAGE.Game.Entity.GetEntityCoords(entity.Handle, false);
                                blip.SetCoords(coords.X, coords.Y, coords.Z);
                            },
                            250,
                            true,
                            0
        ...
        public static ExtraBlip CreateGPS(Vector3 pos, uint dim, bool drawRoute, string notificationAddText = null)
        {
            ...
            Notification.Show(Notification.Types.Success,
                Locale.Get("NOTIFICATION_HEADER_GPS"),
                Locale.Get("BLIP_GEN_GPS_ADDED_0") + (notificationAddText == null ? string.Empty : notificationAddText)
            );
