...
namespace BlaineRP.Client.Game.Fractions
{
    public class EMS : Fraction, IUniformable
    {
        public EMS(FractionTypes type,
                   ...
                   uint metaFlags,
                   string bedPositionsJs) : base(type,
            name,
            storageContainerId,
            containerPos,
            cWbPos,
        ...
        )
        {
            ...
            if (type == FractionTypes.EMS_BLAINE)
            {
                ...
                mainColshapes = new Vector4[]
                {
                    new Vector4(-255.972f, 6321.406f, 33.116f, 45f),
                    new Vector4(1830.509f, 3679.626f, 33.2749f, 45f),
                };
            }
            else if (type == FractionTypes.EMS_LS)
            {
                mainColshapes = new Vector4[]
                {
                    new Vector4(311.2546f, -592.4204f, 42.32737f, 50f),
                };
            }

            Vector3[] bedPositions = RAGE.Util.Json.Deserialize<Vector3[]>(bedPositionsJs);

            foreach (Vector4 x in mainColshapes)
            {
                ...
                cs = new Circle(x.Position, x.RotationZ, false, Utils.Misc.RedColor, Settings.App.Static.MainDimension, null)
                {
                                ...
                                var bedHashes = new List<string>()
                                    {
                                        "v_med_bed2",
                                        "v_med_bed1",
                                    }.Select(x => RAGE.Util.Joaat.Hash(x))
                                     .ToList();

                                for (var i = 0; i < bedPositions.Length; i++)
                                {
                                    Vector3 bedPos = bedPositions[i];

                                    MapObject bedObj = null;

                                    foreach (uint x in bedHashes)
                                    {
                                        int bedHandle = RAGE.Game.Object.GetClosestObjectOfType(bedPos.X, bedPos.Y, bedPos.Z, 1f, x, false, true, true);
                                    ...
                                    bedObj.SetData("CustomText",
                                        (Action<float, float>)((x, y) =>
                                        {
                                            Graphics.DrawText($"Больничная койка",
                                                x,
                                            ...
                                            bool isOccupied = IsBedOccupied(bedIdx);
                    ...
                    OnExit = (cancel) =>
                    {
                        ...
                        if (TempObjects != null)
                        {
                            foreach (MapObject x in TempObjects)
                            {
                                x?.Destroy();
                            }
        ...
        public bool IsBedOccupied(int bedIdx)
        {
            return World.Core.GetSharedData<bool>($"EMS::{(int)Type}::BED::{bedIdx}", false);
        }
        ...
        {
            string[] bedIds = obj.GetData<string>("EmsBedId")?.Split('_');
            ...
            int health = Player.LocalPlayer.GetRealHealth();
    ...
    public class EMSEvents
    {
        public EMSEvents()
        {
            Events.Add("Ems::ShowPlayerDiagnostics",
                async (args) =>
                {
                    ...
                    decimal health = infoObj["hp"].ToObject<decimal>();
                    decimal mood = infoObj["mood"].ToObject<decimal>();
                    decimal satiety = infoObj["satiety"].ToObject<decimal>();

                    decimal drugAddiction = infoObj["da"].ToObject<decimal>();

                    bool isWounded = infoObj["ws"].ToObject<bool>();

                    MedicalCard.DiagnoseTypes preDiagnosisType = infoObj["dType"].ToObject<MedicalCard.DiagnoseTypes>();

                    string text = Locale.Get("EMS_DIAGNOSTICS_TEXT_0",
                        health,
                        mood,
                        satiety,
                        drugAddiction,
                        isWounded ? Locale.Get("GEN_TEXT_YES_0") : Locale.Get("GEN_TEXT_NO_0"),
                        Locale.Get(MedicalCard.GetDiagnoseNameKey(preDiagnosisType))
                    );

                    await ActionBox.ShowText("EMS_SHOWPLAYERDIAGNOSTICS",
                        Locale.Get("EMS_DIAGNOSTICS_TEXT_L", player.GetName(true, false, true)),
                        text,
                        null,
                        null,
