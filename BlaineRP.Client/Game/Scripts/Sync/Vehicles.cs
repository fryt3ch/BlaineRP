    ...
    public class Vehicles
    {
            ...
            Events.Add("Vehicles::JVRO",
                async (args) =>
                {
                    ...
                    await ActionBox.ShowMoney("JobVehicleRentMoney",
                        ...
                        string.Format(Locale.Actions.JobVehicleRentText, $"{vData.Data.Name} [{vehicle.GetNumberplateText() ?? "null"}]", Locale.Get("GEN_MONEY_0", rentPrice)),
                        () =>
                        {
            ...
            _currentDriverSyncTask = new AsyncTask(async () =>
                {
                    while (true)
                    {
                        ...
                        SetColshapeVehicleMaxSpeed(veh,
                            Player.LocalPlayer.HasData("ColshapeVehicleSpeedLimited") ? Player.LocalPlayer.GetData<float>("ColshapeVehicleSpeedLimited") : float.MinValue
                        );
        ...
        public static void ToggleAutoPilot(bool? forceStatus = null, bool stopVehicle = false)
        {
            ...
            else
            {
                ...
                Notification.Show(Notification.Types.Information, Locale.Notifications.Vehicles.Additional.HeaderAutoPilot, Locale.Notifications.Vehicles.Additional.On);
                    ...
                    (rType, id) =>
                    {
                        if (rType == ActionBox.ReplyTypes.OK)
                        {
                            if (id == 0)
                            {
                                ...
                                {
                                    Vector3 wPos = Raycast.FindEntityWaterIntersectionCoord(veh, new Vector3(0f, 0f, 2.5f), 10f, 1.5f, -7.5f, 45f, 0.5f, 31);
        ...
        public static void FixVehicle(Vehicle vehicle)
        {
            if (!vehicle.IsDamaged() && vehicle.GetEngineHealth() >= 1000f && vehicle.GetBodyHealth() >= 1000f)
                Notification.Show(Notification.Types.Information, Locale.Get("NOTIFICATION_HEADER_DEF"), Locale.Notifications.Vehicles.VehicleIsNotDamagedFixError);
            else
        ...
        public static void ToggleCruiseControl(bool ignoreIf = false)
        {
            ...
            if (!ignoreIf)
            {
                ...
                if (speed < Settings.App.Static.MIN_CRUISE_CONTROL_SPEED)
                {
                    Notification.Show(Notification.Types.Error,
                        Locale.Notifications.Vehicles.Additional.HeaderCruise,
                        string.Format(Locale.Notifications.Vehicles.Additional.MinSpeed, System.Math.Floor(Settings.App.Static.MIN_CRUISE_CONTROL_SPEED * 3.6f))
                    );
                ...
                else if (speed > Settings.App.Static.MAX_CRUISE_CONTROL_SPEED)
                {
                    Notification.Show(Notification.Types.Error,
                        Locale.Notifications.Vehicles.Additional.HeaderCruise,
                        string.Format(Locale.Notifications.Vehicles.Additional.MaxSpeed, System.Math.Floor(Settings.App.Static.MAX_CRUISE_CONTROL_SPEED * 3.6f))
                    );
        ...
        public static void BeltTick()
        {
            ...
            if (RAGE.Game.Pad.IsDisabledControlJustPressed(32, 75))
                ...
                {
                    Notification.ShowError("NTFC_VEHICLE_SEATBELT_0");
