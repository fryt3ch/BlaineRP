...
namespace BlaineRP.Client.Game.Scripts
{
    ...
    public class ShootingRange
    {
        ...
        {
            {
                Types.Shop, new Data(new Vector3(-90f, 0f, 160f),
                    6,
                    new Vector3[][]
                    {
                        new Vector3[]
            ...
            task = new AsyncTask(async () =>
                {
                    ...
                    var scaleformCounter = Scaleform.CreateCounter("srange_s_counter",
                        Locale.Get("SCALEFORM_SRANGE_CDOWN_HEADER"),
                        Locale.Get("SCALEFORM_SRANGE_CDOWN_CONTENT"),
                        5,
                        Scaleform.CounterSoundTypes.Deep
                    );
        ...
        private static void Render()
        {
            ...
            Data rData = Ranges[srType];

            Graphics.DrawText(Locale.Get("SCALEFORM_SRANGE_H_EXIT", Input.Core.GetKeyString(RAGE.Ui.VirtualKeys.Escape)),
                0.5f,
                0.950f,
                255,
                255,
            ...
            Graphics.DrawText(Locale.Get("SCALEFORM_SRANGE_SCORE_T", CurrentScore, CurrentMaxScore),
                0.5f,
                0.875f,
                255,
                255,
            ...
            int totalLooseScore = CurrentTotalShots + CurrentLooseScore;

            double totalAccuracy = System.Math.Round((CurrentScore == 0 ? 1 / ((float)totalLooseScore + 1f) : CurrentScore / (float)totalLooseScore) * 100f, 2);
            ...
            string accText = Locale.Get("SCALEFORM_SRANGE_ACC_T", totalAccuracy);
            ...
            if (Targets.Count <= 5 && World.Core.ServerTime.Subtract(LastTargetAdded).TotalMilliseconds >= NewTargetDelay)
            {
                var target = Target.TryCreateNew(srType);

                if (target != null)
                    LastTargetAdded = World.Core.ServerTime;
            }
            ...
            for (var i = 0; i < Targets.Count; i++)
            {
                ...
                if (x.CurrentState == Target.StateTypes.Opening)
                {
                    ...
                    else
                    {
                        ...
                        Vector3 dRot = rData.TargetRotation;
                ...
                {
                    if (World.Core.ServerTime.Subtract(x.CreationTime).TotalMilliseconds >= OldTargetDelay)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            if (World.Core.ServerTime.Subtract(LastShotRegistered).TotalMilliseconds < 100)
                            {
                            ...
                            LastShotRegistered = World.Core.ServerTime;
                        ...
                        else
                        {
                            if (Targets.Where(y =>
                                            x.Index != y.Index &&
                                            x.Row == y.Row &&
                                            y.CurrentState == Target.StateTypes.Idle &&
                                            System.Math.Abs(y.CurrentInterpolation - x.CurrentInterpolation) <= 0.075f
                                        )
                                       .Any())
                            {
                            ...
                            var newPos = Vector3.Lerp(rData.GetLeftRowPosition(x.Row), rData.GetRightRowPosition(x.Row), x.CurrentInterpolation);
        ...
        private static void ShotHandler(Vector3 pos, Player target, Events.CancelEventArgs cancel)
        {
            CurrentTotalShots++;
        }
        ...
        public static void Finish()
        {
            ...
            double accuracy = System.Math.Round(CurrentScore / (float)(CurrentLooseScore + CurrentTotalShots) * 100f, 2);
        ...
        private class Target
        {
            ...
            {
                Data rData = Ranges[ShootingRangeType];

                Vector3 rPos = rData.GetTargetPositionByIdx(Row, Index);

                Object = new MapObject(RAGE.Util.Joaat.Hash(PropVariant == 0 ? "prop_range_target_01" : "prop_range_target_03"),
                    rPos,
                    rData.TargetRotation,
                    75,
                ...
                CurrentInterpolation = rPos.DistanceTo(rData.GetLeftRowPosition(Row)) / rData.GetRowDistance(Row);

                this.Direction = Direction;

                CurrentState = StateTypes.Opening;

                CreationTime = World.Core.ServerTime;
            ...
            public static Target TryCreateNew(Types srType)
            {
                ...
                for (var i = 0; i < rData.TotalRows; i++)
                {
                    int count = Targets.Where(x => x.Row == i).Count();
                ...
                for (var i = 0; i < rData.TargetPositions[row].Length; i++)
                {
                    int idx = rand.Next(0, rData.TargetPositions[row].Length);
        ...
        public class Data
        {
            public Data(Vector3 TargetRotation, int MaxTargetsInRow, Vector3[][] TargetPositions)
            {
                ...
                RowsDistances = new float[TargetPositions.Length];
            ...
            public Vector3 GetTargetPositionByIdx(int row, int idx)
            {
                return TargetPositions[row][idx];
            }

            public Vector3 GetLeftRowPosition(int row)
            {
                return TargetPositions[row][0];
            }

            public Vector3 GetRightRowPosition(int row)
            {
                return TargetPositions[row][TargetPositions[row].Length - 1];
            }

            public float GetRowDistance(int row)
            {
                return RowsDistances[row];
            }
        }
    }
}