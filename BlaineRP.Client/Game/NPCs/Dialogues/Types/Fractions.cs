    ...
    public class Fraction
    {
        public Fraction()
        {
            new Dialogue("cop_0_g", "Здравствуйте, я - дежурный полиции, чем могу помочь?", null,
                new Dialogue.Button("[Купить гос. номер для транспорта]", async () =>
                {
                                    ...
                                    (
                                        ...
                                        async (rType) =>
                                        {
                                            if (rType == ActionBox.ReplyTypes.OK || rType == ActionBox.ReplyTypes.Cancel)
                                            {
                                                ...
                                                if (res != null)
                                                {
                                                    Notification.Show(Notification.Types.Success, Locale.Get("NOTIFICATION_HEADER_DEF"), $"Вы поставили {vData.Item3.Name} на гос. учёт, он зарегистрирован под гос. номером [{res}]");
                                    ...
                                    (
                                        ...
                                        async (rType) =>
                                        {
                                            if (rType == ActionBox.ReplyTypes.OK || rType == ActionBox.ReplyTypes.Cancel)
                                            {
                                                ...
                                                if (res != null)
                                                {
                                                    Notification.Show(Notification.Types.Success, Locale.Get("NOTIFICATION_HEADER_DEF"), $"Вы сняли {vData.Item3.Name} с гос. учёта!");

                                                    if (res.Length > 0)
                                                        Notification.ShowHint($"На транспорте, который вы только что сняли с учёта, сейчас фактически установлен гос. номер - [{res}], если Вы не хотите получать штрафы за езду на транспорте с ложным гос. номером - поставьте его на учёт!", true);
