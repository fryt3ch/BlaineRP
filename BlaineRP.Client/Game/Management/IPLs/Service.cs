...
namespace BlaineRP.Client.Game.Management.IPLs
{
    ...
    public class Service
    {
        ...
        public Service()
        {
            ...
            int pillboxIntId = RAGE.Game.Interior.GetInteriorAtCoords(311.2546f, -592.4204f, 42.32737f);
            ...
            World.Core.AddDataHandler("PRISON_ALARMS",
                async (value, oldValue) =>
                {
                    bool state = (bool?)value ?? false;

                    int prisonIntId = RAGE.Game.Interior.GetInteriorAtCoordsWithType(1787.004f, 2593.1984f, 45.7978f, "int_prison_main");
            ...
            Events.Add("IPL::Switch",
                (object[] args) =>
                {
                    var name = (string)args[0];
                    var state = (bool)args[1];

                    IPLInfo iplInfo = _all.Where(x => x.Name == name).FirstOrDefault();
