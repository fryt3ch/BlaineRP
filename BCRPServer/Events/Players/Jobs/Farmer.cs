...
namespace BCRPServer.Events.Players.Jobs
{
    public class Farmer : Script
    {
        ...
        private static void OrangeTreeFinishCollect(Player player)
        {
            ...
            if (pData.GeneralAnim != Sync.Animations.GeneralTypes.TreeCollect0 || player.Dimension != Properties.Settings.Static.MainDimension || pData.IsCuffed || pData.IsFrozen || pData.IsKnocked || player.Vehicle != null || business.OrangeTrees[treeIdx].Position.DistanceTo(player.Position) > 10f)
            {
                if (pData.GeneralAnim == Sync.Animations.GeneralTypes.TreeCollect0)
                    pData.StopGeneralAnim();
        ...
        private static void CowFinishCollect(Player player)
        {
            ...
            if (pData.GeneralAnim != Sync.Animations.GeneralTypes.MilkCow0 || player.Dimension != Properties.Settings.Static.MainDimension || pData.IsCuffed || pData.IsFrozen || pData.IsKnocked || player.Vehicle != null || business.Cows[cowIdx].Position.Position.DistanceTo(player.Position) > 10f)
            {
                if (pData.GeneralAnim == Sync.Animations.GeneralTypes.MilkCow0)
                    pData.StopGeneralAnim();
