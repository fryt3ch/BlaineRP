...
namespace BCRPServer.Events.Players.Jobs
{
    public class Farmer : Script
    {
        ...
        private static byte CropProcess(Player player, int fieldIdx, byte col, byte row)
        {
            ...
            if (player.Dimension != Settings.MAIN_DIMENSION || pData.IsCuffed || pData.IsFrozen || pData.IsKnocked || player.Vehicle != null || pData.HasAnyHandAttachedObject || pData.IsAnyAnimOn())
                return 0;
        ...
        private static void OrangeTreeFinishCollect(Player player)
        {
            ...
            if (pData.GeneralAnim != Sync.Animations.GeneralTypes.TreeCollect0 || player.Dimension != Settings.MAIN_DIMENSION || pData.IsCuffed || pData.IsFrozen || pData.IsKnocked || player.Vehicle != null || business.OrangeTrees[treeIdx].Position.DistanceTo(player.Position) > 10f)
            {
                if (pData.GeneralAnim == Sync.Animations.GeneralTypes.TreeCollect0)
                    pData.StopGeneralAnim();
        ...
        private static void OrangeTreeFinish(Player player, int boxIdx)
        {
            ...
            {
                if (player.Dimension != Settings.MAIN_DIMENSION || pData.IsCuffed || pData.IsFrozen || pData.IsKnocked || player.Vehicle != null || farmBusiness.OrangeTreeBoxPositions[boxIdx].DistanceTo(player.Position) > 5f)
                    return;
        ...
        private static void CowFinishCollect(Player player)
        {
            ...
            if (pData.GeneralAnim != Sync.Animations.GeneralTypes.MilkCow0 || player.Dimension != Settings.MAIN_DIMENSION || pData.IsCuffed || pData.IsFrozen || pData.IsKnocked || player.Vehicle != null || business.Cows[cowIdx].Position.Position.DistanceTo(player.Position) > 10f)
            {
                if (pData.GeneralAnim == Sync.Animations.GeneralTypes.MilkCow0)
                    pData.StopGeneralAnim();
        ...
        private static void CowFinish(Player player, int bucketIdx)
        {
            ...
            {
                if (player.Dimension != Settings.MAIN_DIMENSION || pData.IsCuffed || pData.IsFrozen || pData.IsKnocked || player.Vehicle != null || farmBusiness.CowBucketPositions[bucketIdx].DistanceTo(player.Position) > 5f)
                    return;
