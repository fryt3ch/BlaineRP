...
namespace BCRPServer.Events.Players.Misc
{
    internal class Other : Script
    {
        ...
        public static void NonRpPrisonTeleportMe(Player player)
        {
            ...
            if (pData.Player.Dimension != Properties.Settings.Profile.Current.Game.DemorganDimension)
                return;
        ...
        public static string EstateAgencyGetData(Player player, int estAgencyId, int posId)
        {
            ...
            if (player.Dimension != Properties.Settings.Profile.Current.Game.MainDimension || player.Position.DistanceTo(estAgency.Positions[posId]) > 5f)
                return null;
