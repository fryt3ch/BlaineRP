        ...
        public static void ExitFromBuiness(PlayerData pData, bool teleport = true)
        {
            ...
            if (business is Game.Businesses.IEnterable enterable)
            {
                ...
                if (business is Game.Businesses.TuningShop ts)
                {
                    ...
                    {
                        if (teleport)
                            player.Teleport(t.Position, false, Settings.CurrentProfile.Game.MainDimension, t.RotationZ, true);
                    }
                    else
                    {
                        if (player.Vehicle != veh.Vehicle || !teleport)
                        {
                            ...
                            veh.Vehicle.Teleport(t.Position, Settings.CurrentProfile.Game.MainDimension, t.RotationZ, false, Additional.AntiCheat.VehicleTeleportTypes.Default);

                            veh.AttachBoatToTrailer();
                        }
                        else
                        {
                            veh.Vehicle.Teleport(t.Position, Settings.CurrentProfile.Game.MainDimension, t.RotationZ, true, Additional.AntiCheat.VehicleTeleportTypes.OnlyDriver);
                ...
                {
                    if (teleport)
                        player.Teleport(t.Position, true, Settings.CurrentProfile.Game.MainDimension, t.RotationZ, true);
