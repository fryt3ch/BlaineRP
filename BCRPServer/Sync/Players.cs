        ...
        public static void ExitFromBuiness(PlayerData pData, bool teleport = true)
        {
            ...
            if (business is Game.Businesses.IEnterable enterable)
            {
                ...
                if (business is Game.Businesses.TuningShop ts)
                {
                    ...
                    {
                        if (teleport)
                            player.Teleport(t.Position, false, Settings.MAIN_DIMENSION, t.RotationZ, true);
                    }
                    else
                    {
                        if (player.Vehicle != veh.Vehicle || !teleport)
                        {
                            ...
                            veh.Vehicle.Teleport(t.Position, Settings.MAIN_DIMENSION, t.RotationZ, false, Additional.AntiCheat.VehicleTeleportTypes.Default);

                            veh.AttachBoatToTrailer();
                        }
                        else
                        {
                            veh.Vehicle.Teleport(t.Position, Settings.MAIN_DIMENSION, t.RotationZ, true, Additional.AntiCheat.VehicleTeleportTypes.OnlyDriver);
                ...
                {
                    if (teleport)
                        player.Teleport(t.Position, true, Settings.MAIN_DIMENSION, t.RotationZ, true);
