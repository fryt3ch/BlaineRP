...

                {
                    ...
                    new Quest.QuestData.StepData("Заберите деньги", 1)
                    {
                        StartAction = (pData, quest) =>
                        {
                            ...
                            var currentOrder = new Data.Jobs.Collector.OrderInfo() { Id = uint.Parse(qData[0]), TargetBusiness = BCRPClient.Data.Locations.Business.All[int.Parse(qData[1])], Reward = uint.Parse(qData[2]) };
